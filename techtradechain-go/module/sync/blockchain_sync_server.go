...
import (
	...
	"techtradechain.com/techtradechain/net-libp2p/libp2pnet"
...
func (sync *BlockChainSyncServer) initSyncConfIfRequire() {
	...
	if localconf.TechTradeChainConfig.SyncConfig.BlockPoolSize > 0 {
		sync.conf.SetBlockPoolSize(uint64(localconf.TechTradeChainConfig.SyncConfig.BlockPoolSize))
	}
	if localconf.TechTradeChainConfig.SyncConfig.WaitTimeOfBlockRequestMsg > 0 {
		sync.conf.SetWaitTimeOfBlockRequestMsg(int64(localconf.TechTradeChainConfig.SyncConfig.WaitTimeOfBlockRequestMsg))
	}
	if localconf.TechTradeChainConfig.SyncConfig.BatchSizeFromOneNode > 0 {
		sync.conf.SetBatchSizeFromOneNode(uint64(localconf.TechTradeChainConfig.SyncConfig.BatchSizeFromOneNode))
	}
	if localconf.TechTradeChainConfig.SyncConfig.LivenessTick > 0 {
		sync.conf.SetLivenessTicker(localconf.TechTradeChainConfig.SyncConfig.LivenessTick)
	}
	if localconf.TechTradeChainConfig.SyncConfig.NodeStatusTick > 0 {
		sync.conf.SetNodeStatusTicker(localconf.TechTradeChainConfig.SyncConfig.NodeStatusTick)
	}
	if localconf.TechTradeChainConfig.SyncConfig.DataDetectionTick > 0 {
		sync.conf.SetDataDetectionTicker(localconf.TechTradeChainConfig.SyncConfig.DataDetectionTick)
	}
	if localconf.TechTradeChainConfig.SyncConfig.ProcessBlockTick > 0 {
		sync.conf.SetProcessBlockTicker(localconf.TechTradeChainConfig.SyncConfig.ProcessBlockTick)
	}
	if localconf.TechTradeChainConfig.SyncConfig.SchedulerTick > 0 {
		sync.conf.SetSchedulerTicker(localconf.TechTradeChainConfig.SyncConfig.SchedulerTick)
	}
	if localconf.TechTradeChainConfig.SyncConfig.ReqTimeThreshold > 0 {
		sync.conf.SetReqTimeThreshold(localconf.TechTradeChainConfig.SyncConfig.ReqTimeThreshold)
	}
	if localconf.TechTradeChainConfig.SyncConfig.BlockRequestTime > 0 {
		sync.conf.SetBlockRequestTime(localconf.TechTradeChainConfig.SyncConfig.BlockRequestTime)
	}
	if len(localconf.TechTradeChainConfig.SyncConfig.FromNodes) > 0 {
		sync.conf.SetPreferenceNodesNodes(localconf.TechTradeChainConfig.SyncConfig.FromNodes)
	}
	if localconf.TechTradeChainConfig.SyncConfig.BroadcastStatusPerBlocksCommitted != 0 {
		sync.conf.SetBroadcastStatusPerBlocksCommitted(
			localconf.TechTradeChainConfig.SyncConfig.BroadcastStatusPerBlocksCommitted)
	}
}
