...
import (
	...
	"techtradechain.com/techtradechain-go/module/core"
	"techtradechain.com/techtradechain-go/module/core/cache"
	providerConf "techtradechain.com/techtradechain-go/module/core/provider/conf"
	...
	blockSync "techtradechain.com/techtradechain-go/module/sync"
	"techtradechain.com/techtradechain-go/module/txfilter"
	"techtradechain.com/techtradechain-go/module/txfilter/filtercommon"
	"techtradechain.com/techtradechain-go/module/txpool"
	componentVm "techtradechain.com/techtradechain-go/module/vm"
...
func (bc *Blockchain) initStore() (err error) {
	...
	config, err := conf.NewStorageConfig(localconf.TechTradeChainConfig.StorageConfig)
	//err = mapstructure.Decode(localconf.TechTradeChainConfig.StorageConfig, config)
	if err != nil {
		return err
	}

	//p11Handle, err := localconf.TechTradeChainConfig.GetP11Handle()
...
func (bc *Blockchain) initOldStore() (err error) {
	...
	err = mapstructure.Decode(localconf.TechTradeChainConfig.StorageConfig, config)
	timeS, _ := localconf.TechTradeChainConfig.StorageConfig["back_path"].(string)
...
func (bc *Blockchain) initChainConf() (err error) {
	...
	localAuthType := strings.ToLower(localconf.TechTradeChainConfig.AuthType)
...
func (bc *Blockchain) initAC() (err error) {
	...
	nodeConfig := localconf.TechTradeChainConfig.NodeConfig
...
func (bc *Blockchain) initTxPool() (err error) {
	...
	if value, ok := localconf.TechTradeChainConfig.TxPoolConfig["pool_type"]; ok {
	...
	currentTxPool, err := txPoolProvider(
		localconf.TechTradeChainConfig.NodeConfig.NodeId,
		bc.chainId,
		bc.txFilter,
		bc.store,
		bc.msgBus,
		...
		bc.netService,
		txPoolLogger,
		localconf.TechTradeChainConfig.MonitorConfig.Enabled,
		localconf.TechTradeChainConfig.TxPoolConfig,
	)
	...
	if bc.netService == nil {
		/*
			bc.vmMgr = vm.NewVmManager(
				...
				localconf.TechTradeChainConfig.GetStorePath(),
...
func (bc *Blockchain) initConsensus() (err error) {
	...
	id := localconf.TechTradeChainConfig.NodeConfig.NodeId
