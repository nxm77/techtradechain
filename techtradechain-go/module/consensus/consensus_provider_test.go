...
func TestNewConsensusEngine(t *testing.T) {
	...
	prePath := localconf.TechTradeChainConfig.GetStorePath()
	defer func() {
		localconf.TechTradeChainConfig.StorageConfig["store_path"] = prePath
	}()
	localconf.TechTradeChainConfig.StorageConfig["store_path"] = filepath.Join(os.TempDir(), fmt.Sprintf("%d", time.Now().Nanosecond()))
