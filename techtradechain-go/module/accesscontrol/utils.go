...
func getHSMHandleId() string {
	p11Config := localconf.TechTradeChainConfig.NodeConfig.P11Config
...
func getHSMHandle() (interface{}, error) {
	var err error
	cfg := localconf.TechTradeChainConfig.NodeConfig.P11Config
...
// @return error
func initKMS() error {
	config := localconf.TechTradeChainConfig.NodeConfig.KMSConfig
	if !config.Enabled {
...
func InitCertSigningMember(chainConfig *config.ChainConfig, localOrgId,
	...
	if localPrivKeyFile != "" && localCertFile != "" {
		...
		cfg := localconf.TechTradeChainConfig.NodeConfig
