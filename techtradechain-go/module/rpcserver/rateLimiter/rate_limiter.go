...
func NewSubscriberRateLimiter(logger *logger.CMLogger) SubscriberRateLimiter {
	...
	if !localconf.TechTradeChainConfig.RpcConfig.SubscriberConfig.RateLimitConfig.Enabled {
		return nil
	}

	// Determine the rate limiter type from the configuration
	rateLimiterType := localconf.TechTradeChainConfig.RpcConfig.RateLimitConfig.Type
...
func newRateLimiter() *rate.Limiter {
	...
	tokenBucketSize := localconf.TechTradeChainConfig.RpcConfig.SubscriberConfig.RateLimitConfig.TokenBucketSize
	tokenPerSecond := localconf.TechTradeChainConfig.RpcConfig.SubscriberConfig.RateLimitConfig.TokenPerSecond
