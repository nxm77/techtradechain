...
import (
	...
	"techtradechain.com/techtradechain-go/module/subscriber/model"
...
func NewRootDispatcher(techTradeChainServer *blockchain.TechTradeChainServer) (*RootDispatcher, error) {
	childs := make(childDispatchers)
	for _, chainConfig := range localconf.TechTradeChainConfig.BlockChainConfig {
		eventSubscriber, err := techTradeChainServer.GetEventSubscribe(chainConfig.ChainId)
		if err != nil {
			return nil, err
		}
...
func (root *RootDispatcher) CheckAndUpdate() error {
	...
	for _, chainConfig := range localconf.TechTradeChainConfig.BlockChainConfig {
		...
		if _, exists := oldChilds[chainId]; !exists {
			eventSubscriber, err := root.techTradeChainServer.GetEventSubscribe(chainId)
			if err != nil {
				return err
			}
