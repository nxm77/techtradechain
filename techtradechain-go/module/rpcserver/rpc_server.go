...
import (
	...
	"techtradechain.com/techtradechain/common/v2/monitor"
...
type RPCServer struct {
	...
	techTradeChainServer           *blockchain.TechTradeChainServer
	log                        *logger.CMLogger
	ctx                        context.Context
...
func NewRPCServer(techTradeChainServer *blockchain.TechTradeChainServer) (*RPCServer, error) {

	grpcServer, err := newGrpc(techTradeChainServer)
	if err != nil {
		return nil, fmt.Errorf("new grpc server failed, %s", err.Error())
	}

	mixServer, err := newMixServer(grpcServer, techTradeChainServer)
	if err != nil {
		return nil, fmt.Errorf("new http grpc server failed, %s", err.Error())
	}

	dispatcher, err = NewRootDispatcher(techTradeChainServer)
	if err != nil {
		return nil, fmt.Errorf("new root dispatcher failed, %s", err.Error())
	}
	dispatcher.Start()

	if localconf.TechTradeChainConfig.MonitorConfig.Enabled {
	...
	return &RPCServer{
		...
		mixServer:        mixServer,
		techTradeChainServer: techTradeChainServer,
...
func (s *RPCServer) Start() error {
	...
	endPoint := fmt.Sprintf("%s:%d", localconf.TechTradeChainConfig.RpcConfig.Host,
		localconf.TechTradeChainConfig.RpcConfig.Port)
...
func (s *RPCServer) checkAndReloadChainConfTrustRoots() error {
	var err error
	if localconf.TechTradeChainConfig.RpcConfig.TLSConfig.Mode != TLS_MODE_DISABLE {
...
func (s *RPCServer) configureTLS() (*cmtls.Config, error) {
	...
	var err error
	var caCerts []string
	if strings.ToLower(localconf.TechTradeChainConfig.AuthType) == protocol.PermissionedWithKey ||
		strings.ToLower(localconf.TechTradeChainConfig.AuthType) == protocol.Public {
		if localconf.TechTradeChainConfig.RpcConfig.TLSConfig.Mode != TLS_MODE_DISABLE {
			if localconf.TechTradeChainConfig.RpcConfig.TLSConfig.Mode == TLS_MODE_TWOWAY {
				var certs []string
				certs, err = loadCerts(
					localconf.TechTradeChainConfig.RpcConfig.TLSConfig.ClientRootCaPaths)
				if err != nil {
					return nil, err
				}
			...
			tlsConfig, err = ca.GetTLSConfig(localconf.TechTradeChainConfig.RpcConfig.TLSConfig.CertFile,
				localconf.TechTradeChainConfig.RpcConfig.TLSConfig.PrivKeyFile, []string{}, caCerts,
				localconf.TechTradeChainConfig.RpcConfig.TLSConfig.CertEncFile,
				localconf.TechTradeChainConfig.RpcConfig.TLSConfig.PrivEncKeyFile)
		...
		if localconf.TechTradeChainConfig.RpcConfig.TLSConfig.Mode != TLS_MODE_DISABLE {
			if localconf.TechTradeChainConfig.RpcConfig.TLSConfig.Mode == TLS_MODE_TWOWAY {
				caCerts, err = getCACerts(s.techTradeChainServer)
				if err != nil {
					return nil, err
				}
			}
			...
			if localconf.TechTradeChainConfig.RpcConfig.TLSConfig.Mode == TLS_MODE_TWOWAY {
				...
				acs, err = s.techTradeChainServer.GetAllAC()
				if err != nil {
...
func (s *RPCServer) RegisterHandler() error {
	apiService := NewApiService(s.ctx, s.techTradeChainServer)
...
func (s *RPCServer) Restart(reason string) error {
	...
	s.grpcServer, err = newGrpc(s.techTradeChainServer)
	if err != nil {
	...
	s.mixServer, err = newMixServer(s.grpcServer, s.techTradeChainServer)
	if err != nil {
...
func (s *RPCServer) getCurChainConfTrustRootsHash() (string, error) {
	chainConfs, err := s.techTradeChainServer.GetAllChainConf()
	if err != nil {
...
func (s *RPCServer) sleep() {
	checkChainConfTrustRootsChangeInterval :=
		localconf.TechTradeChainConfig.RpcConfig.CheckChainConfTrustRootsChangeInterval
...
func newGrpc(techTradeChainServer *blockchain.TechTradeChainServer) (*grpc.Server, error) {
	...
	if strings.ToLower(localconf.TechTradeChainConfig.AuthType) == protocol.PermissionedWithKey ||
		...
		if localconf.TechTradeChainConfig.RpcConfig.TLSConfig.Mode != TLS_MODE_DISABLE {
			var caCerts []string
			certs, err := loadCerts(localconf.TechTradeChainConfig.RpcConfig.TLSConfig.ClientRootCaPaths)
			if err != nil {
				return nil, err
			}
			for _, certFile := range certs {
			...
			tlsRPCServer := ca.CAServer{
				CaCerts:  caCerts,
				CertFile: localconf.TechTradeChainConfig.RpcConfig.TLSConfig.CertFile,
				KeyFile:  localconf.TechTradeChainConfig.RpcConfig.TLSConfig.PrivKeyFile,
				Logger:   log,
			}
		...
		if localconf.TechTradeChainConfig.RpcConfig.TLSConfig.Mode != TLS_MODE_DISABLE {
			...
			caCerts, err := getCACerts(techTradeChainServer)
			if err != nil {
				return nil, err
			}
			...
			acs, err := techTradeChainServer.GetAllAC()
			if err != nil {
				log.Errorf("get all AccessControlProvider failed, %s", err.Error())
				return nil, err
			}
	...
	opts = append(opts, grpc.MaxSendMsgSize(localconf.TechTradeChainConfig.RpcConfig.MaxSendMsgSize))
	opts = append(opts, grpc.MaxRecvMsgSize(localconf.TechTradeChainConfig.RpcConfig.MaxRecvMsgSize))
...
func newMixServer(grpcServer *grpc.Server, techTradeChainServer *blockchain.TechTradeChainServer) (*http.Server, error) {
	...
	if localconf.TechTradeChainConfig.RpcConfig.GatewayConfig.Enabled {
		mux = http.NewServeMux()
		gwmux, err := newGateway(techTradeChainServer)
		if err != nil {
			log.Error(err)
			return nil, err
		}
	...
	if localconf.TechTradeChainConfig.RpcConfig.GatewayConfig.Enabled {
		httpServer = &http.Server{
			Handler: wsproxy.WebsocketProxy(handler, wsproxy.WithMaxRespBodyBufferSize(
				localconf.TechTradeChainConfig.RpcConfig.GatewayConfig.MaxRespBodySize*1024*1024)),
...
func newGateway(techTradeChainServer *blockchain.TechTradeChainServer) (http.Handler, error) {
	...
	endPoint := fmt.Sprintf("%s:%d", "127.0.0.1", localconf.TechTradeChainConfig.RpcConfig.Port)
...
func getCACerts(techTradeChainServer *blockchain.TechTradeChainServer) ([]string, error) {
	chainConfs, err := techTradeChainServer.GetAllChainConf()
	if err != nil {
