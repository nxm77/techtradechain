...
import (
	...
	"techtradechain.com/techtradechain-go/module/rpcserver/rateLimiter"

	"techtradechain.com/techtradechain/pb-go/v2/consensus"
	"google.golang.org/protobuf/types/known/wrapperspb"

	"techtradechain.com/techtradechain-go/module/blockchain"
	"techtradechain.com/techtradechain-go/module/snapshot"
	...
	txpoolPb "techtradechain.com/techtradechain/pb-go/v2/txpool"
...
func NewApiService(ctx context.Context, techTradeChainServer *blockchain.TechTradeChainServer) *ApiService {
...
func (s *ApiService) validate(tx *commonPb.Transaction) (errCode commonErr.ErrCode, errMsg string) {
	...
	chainConfig, err := s.techTradeChainServer.GetChainConf(tx.Payload.ChainId)
	if err != nil {
	...
	if localconf.TechTradeChainConfig.NodeConfig.CertKeyUsageCheck {
	...
	bc, err = s.techTradeChainServer.GetBlockchain(tx.Payload.ChainId)
	if err != nil {
...
func (s *ApiService) dealQuery(tx *commonPb.Transaction, source protocol.TxSource) *commonPb.TxResponse {
	...
	if store, err = s.techTradeChainServer.GetStore(chainId); err != nil {
	...
	if vmMgr, err = s.techTradeChainServer.GetVmManager(chainId); err != nil {
	...
	if cc, err1 := s.techTradeChainServer.GetChainConf(tx.Payload.ChainId); err1 == nil {
...
func (s *ApiService) dealSystemChainQuery(tx *commonPb.Transaction, vmMgr protocol.VmManager) *commonPb.TxResponse {
	...
	//defaultGas := uint64(0)
	//chainConfig, _ := s.techTradeChainServer.GetChainConf(chainId)
...
func (s *ApiService) dealTransact(ctx context.Context, tx *commonPb.Transaction, source protocol.TxSource,
	...
	err = s.techTradeChainServer.AddTx(tx.Payload.ChainId, tx, source)
	...
	if syncResult {
		timeout := time.Duration(localconf.TechTradeChainConfig.RpcConfig.SyncTxResultTimeout) * time.Second
...
// GetTechTradeChainVersion get techtradechain version by rpc request
func (s *ApiService) GetTechTradeChainVersion(ctx context.Context, req *configPb.TechTradeChainVersionRequest) (
	*configPb.TechTradeChainVersionResponse, error) {

	return &configPb.TechTradeChainVersionResponse{
		Code:    int32(0),
		Version: s.techTradeChainServer.Version(),
	}, nil
}
...
func (s *ApiService) GetPoolStatus(ctx context.Context,
	...
	return s.techTradeChainServer.GetPoolStatus(request.ChainId)
}
...
func (s *ApiService) GetTxIdsByTypeAndStage(ctx context.Context,
	...
	txIds, err := s.techTradeChainServer.GetTxIdsByTypeAndStage(request.ChainId,
...
func (s *ApiService) GetTxsInPoolByTxIds(ctx context.Context,
	...
	txs, txIds, err := s.techTradeChainServer.GetTxsInPoolByTxIds(request.ChainId, request.TxIds)
	if err != nil {
		return nil, err
	}
...
func (s *ApiService) GetConsensusStateJSON(ctx context.Context,
	...
	bz, err := s.techTradeChainServer.GetConsensusStateJSON(request.ChainId)
	if err != nil {
		return nil, err
	}
...
func (s *ApiService) GetConsensusValidators(ctx context.Context,
	...
	nodes, err := s.techTradeChainServer.GetConsensusValidators(request.ChainId)
	if err != nil {
		return nil, err
	}
...
func (s *ApiService) GetConsensusHeight(ctx context.Context,
	...
	height, err := s.techTradeChainServer.GetConsensusHeight(request.ChainId)
	if err != nil {
		return nil, err
	}
...
func (s *ApiService) GetSyncState(ctx context.Context, request *syncPb.GetSyncStateRequest) (*syncPb.SyncState, error) {
	syncService, err := s.techTradeChainServer.GetSync(request.ChainId)
	if err != nil {
		return nil, err
	}
