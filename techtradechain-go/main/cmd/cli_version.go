...
func VersionCMD() *cobra.Command {
	return &cobra.Command{
		Use:   "version",
		Short: "Show TechTradeChain version",
		Long:  "Show TechTradeChain version",
...
func logo() string {
	fig := figure.NewFigure("TechTradeChain", "slant", true)
	s := fig.String()
	fragment := "================================================================================="
	//versionInfo := "::TechTradeChain::  version(" + protocol.DefaultBlockVersion + ")"
	// blockchain.CurrentVersion 为对外发布的版本，仅支持三位，如：v2.3.3
	versionInfo := fmt.Sprintf("TechTradeChain Version: %s\n", blockchain.CurrentVersion)
...
func PrintVersion() {
	//fmt.Printf("TechTradeChain version: %s\n", CurrentVersion)
	fmt.Println(logo())
	fmt.Println()
}
