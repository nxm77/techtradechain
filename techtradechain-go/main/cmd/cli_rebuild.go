...
func RebuildDbsCMD() *cobra.Command {
	rebuildDbsCmd := &cobra.Command{
		Use:   "rebuild-dbs",
		Short: "RebuildDbs TechTradeChain",
		Long:  "RebuildDbs TechTradeChain",
...
func backupDbs(chainId string, needVerify bool) {
	...
	localconf.TechTradeChainConfig.StorageConfig["back_path"] = timeS
	localconf.TechTradeChainConfig.StorageConfig["rebuild_chainId"] = chainId
	localconf.TechTradeChainConfig.StorageConfig["need_verify"] = needVerify
	...
	errThenExit(mapstructure.Decode(localconf.TechTradeChainConfig.StorageConfig, config))
...
func rebuildDbsStart() {
	...
	chainId, _ := localconf.TechTradeChainConfig.StorageConfig["rebuild_chainId"].(string)
	needVeri, _ := localconf.TechTradeChainConfig.StorageConfig["need_verify"].(bool)
	if err := techTradeChainServer.InitForRebuildDbs(chainId); err != nil {
	...
	if err := techTradeChainServer.StartForRebuildDbs(needVeri); err != nil {
