...
import (
	...
	"techtradechain.com/techtradechain/vm-wxvm/v2/xvm/compile"
	"techtradechain.com/techtradechain/vm-wxvm/v2/xvm/exec"
	"techtradechain.com/techtradechain/vm-wxvm/v2/xvm/runtime/emscripten"
	"techtradechain.com/techtradechain/vm-wxvm/v2/xvm/runtime/wasi"
...
func NewCodeManager(chainId string, basedir string, log protocol.Logger) *CodeManager {
	...
	if err != nil {
		log.Infof("New code manager failed err: %v", err)
	}
...
func (c *CodeManager) makeMemCache(contractKeyId string, libPath string,
	...
	if err != nil {
		if contextService.ctxMap[contextService.ctxId].TxSimContext.GetBlockVersion() >= 2201 {
			contractDir := path.Dir(libPath)
...
func (c *CodeManager) GetExecCode(chainId string, contract *commonPb.Contract,
	...
	libPath, err, _ := c.makeCacheLock.Do(contractKeyId, func() (interface{}, error) {
	...
	return c.makeMemCache(contractKeyId, libPath.(string), contextService)
}
