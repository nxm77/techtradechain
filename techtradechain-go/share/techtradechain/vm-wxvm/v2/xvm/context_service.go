...
func (c *ContextService) MakeContext(contract *commonPb.Contract, txSimContext protocol.TxSimContext,
	...
	ctx := &Context{
		...
		//ContractEvent:   contractResult.ContractEvent,
...
func (c *ContextService) CallContract(ctxId int64) int32 {
	...
	contractId, err := context.TxSimContext.GetContractByName(contract)
	if err != nil {
		context.err = fmt.Errorf("get contract by name[%v] failed", contract)
		return protocol.ContractSdkSignalResultFail
	}

	var contractResult *commonPb.ContractResult
	var txStatusCode commonPb.TxStatusCode
	if context.TxSimContext.GetBlockVersion() < 2300 {
		contractResult, _, txStatusCode = context.TxSimContext.CallContract(
			context.ContractId,
			&commonPb.Contract{Name: contract},
			method,
			nil,
			paramMap,
			context.gasUsed,
		...
		context.ContractEvent = append(context.ContractEvent, contractResult.ContractEvent...)
	}
