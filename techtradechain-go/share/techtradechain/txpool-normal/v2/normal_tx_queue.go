...
type txQueue struct {
	...
	log       protocol.Logger
	logHelper *LogHelper //Batch printing tx pool full problem
}
...
func newTxQueue(queueNum int, chainConf protocol.ChainConf, blockStore protocol.BlockchainStore,
	...
	logHelper := newLogHelper(time.Second, 30, log, "TxPool is full, txIds:[%s]")
	logHelper.start()
	// create txQueue
	return &txQueue{
...
func (queue *txQueue) isFull(tx *commonPb.Transaction) bool {
	...
	if isConfigTx(tx, queue.chainConf) {
		...
		if configTxSize >= int32(MaxConfigTxPoolSize()) {
			queue.logHelper.append(tx.Payload.GetTxId())
