...
import (
	...
	"techtradechain.com/techtradechain/consensus-maxbft/v2/epoch"
...
const notConsensusNode = "the current node does not belong to the consensus role"
...
type Maxbft struct {
	...
	// and avoid concurrent reading and writing to m.epoch
...
func New(cfg *consensusUtils.ConsensusImplConfig) (protocol.ConsensusEngine, error) {
	...
	// lock to protect m.epoch because the state interface maybe read it
	m.modeLock.Lock()
	m.epoch = currEpoch
	m.modeLock.Unlock()
...
// GetValidators returns the consensus validators list
func (m *Maxbft) GetValidators() ([]string, error) {
	m.modeLock.RLock()
	defer m.modeLock.RUnlock()
	if m.epoch == nil || m.epoch.GetEngine() == nil {
		return nil, fmt.Errorf(notConsensusNode)
	}
	return m.epoch.GetEngine().GetValidators()
}
...
func (m *Maxbft) GetLastHeight() uint64 {
	...
	return m.epoch.GetEngine().GetLastHeight()
}
...
func (m *Maxbft) GetConsensusStateJSON() ([]byte, error) {
	...
	return m.epoch.GetEngine().GetConsensusStateJSON()
}
...
func VerifyBlockSignatures(chainConf protocol.ChainConf, ac protocol.AccessControlProvider,
	...
	_, err = verifier.VerifyVotes(
		ac, len(blockQc.Votes), blockQc.Votes, validators, block.GetHeader().GetBlockVersion())
	return err
}
