...
import (
	...
	conf "techtradechain.com/techtradechain/chainconf/v2"
	"techtradechain.com/techtradechain/common/v2/msgbus"
	"techtradechain.com/techtradechain/consensus-maxbft/v2/compensator"
	"techtradechain.com/techtradechain/consensus-maxbft/v2/forest"
	"techtradechain.com/techtradechain/consensus-maxbft/v2/msgcache"
	"techtradechain.com/techtradechain/consensus-maxbft/v2/pacemaker"
	"techtradechain.com/techtradechain/consensus-maxbft/v2/utils"
	"techtradechain.com/techtradechain/consensus-maxbft/v2/verifier"
	"techtradechain.com/techtradechain/consensus-maxbft/v2/vote"
	"techtradechain.com/techtradechain/consensus-maxbft/v2/wal"
	consensusUtils "techtradechain.com/techtradechain/consensus-utils/v2"
	"techtradechain.com/techtradechain/consensus-utils/v2/consistent_service"
	...
	"techtradechain.com/techtradechain/pb-go/v2/net"
...
// GetValidators returns the consensus validators
func (e *Engine) GetValidators() ([]string, error) {
	return e.nodes, nil
}

// GetLastHeight returns the last height of the consensus
func (e *Engine) GetLastHeight() uint64 {
	return e.forest.GetGenericHeight()
}

// GetConsensusStateJSON returns the consensus state json
func (e *Engine) GetConsensusStateJSON() ([]byte, error) {
	event := e.paceMaker.GetMonitorEvent()
	status := struct {
		Height          uint64
		View            uint64
		CurrViewTimeOut string
		Nodes           []string
		Proposer        string
	}{
		Height:          e.GetLastHeight(),
		View:            event.View,
		CurrViewTimeOut: event.Duration.String(),
		Nodes:           e.nodes,
		Proposer:        e.validator.GetLeader(event.View),
	}

	bz, err := json.Marshal(status)
	if err != nil {
		return nil, fmt.Errorf("marshal consensus state failed, reason: %s", err)
	}
	return bz, nil
}
...
func (e *Engine) initState() error {
	...
	walPath := path.Join(localconf.TechTradeChainConfig.GetStorePath(), e.chainID,
