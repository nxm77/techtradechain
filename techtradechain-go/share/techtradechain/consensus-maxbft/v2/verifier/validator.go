...
import (
	...
	"techtradechain.com/techtradechain/consensus-maxbft/v2/forest"
	"techtradechain.com/techtradechain/consensus-maxbft/v2/utils"
	"techtradechain.com/techtradechain/pb-go/v2/common"
	"techtradechain.com/techtradechain/pb-go/v2/consensus"
	"techtradechain.com/techtradechain/pb-go/v2/consensus/maxbft"
	maxbftpb "techtradechain.com/techtradechain/pb-go/v2/consensus/maxbft"
...
type Verifier interface {
	...
	ValidateQc(qc *maxbftpb.QuorumCert, blockVersion uint32) error
...
func (v *Validator) ValidateQc(qc *maxbftpb.QuorumCert, blockVersion uint32) error {
	...
	if err := v.checkVotesInQc(qc, blockVersion); err != nil {
...
func (v *Validator) ValidateProposal(proposal *maxbftpb.ProposalData, qc *maxbftpb.QuorumCert,
	...
	if mode == protocol.CONSENSUS_VERIFY || len(proposal.TxRwSet) == 0 || qc == nil {
		result, err = v.verifier.VerifyBlockSync(proposal.Block, protocol.CONSENSUS_VERIFY)
		// we should make sure that the proposals that has been committed
		// can still update the forest, so that the children proposals
		// can be validated, so we cannot return the error here
		if err == commonErrors.ErrBlockHadBeenCommited {
			v.logger.Infof("the block in the proposal[%d:%d:%x] has been committed, not need to process",
		...
		err = v.ValidateQc(qc, proposal.GetBlock().GetHeader().GetBlockVersion())
		if err != nil {
			err = fmt.Errorf("ValidateQc failed. error:%+v", err)
			return nil, err
		}
		err = v.verifier.VerifyBlockWithRwSets(proposal.Block, proposal.TxRwSet, mode)
		if err == commonErrors.ErrBlockHadBeenCommited {
			v.logger.Infof("the block in the proposal[%d:%d:%x] has been committed, not need to process",
				proposal.Block.Header.BlockHeight, proposal.View, proposal.Block.Header.BlockHash)
			return nil, nil
...
func (v *Validator) ValidateVote(vote *maxbftpb.VoteData, proposal *maxbftpb.ProposalData) error {
	...
	blockVersion := proposal.GetBlock().GetHeader().GetBlockVersion()
	if err = v.validateSignature(vote, blockVersion); err != nil {
...
func (v *Validator) validateSignature(vote *maxbftpb.VoteData, blockVersion uint32) error {
	if err := verifyConsensusMsgSign(vote, v.ac, blockVersion); err != nil {
...
func (v *Validator) checkVotesInQc(qc *maxbftpb.QuorumCert, blockVersion uint32) error {
	// check every vote in qc, and calculate the valid vote count
	votedBlockNum, err := v.countNumFromVotes(qc, blockVersion)
	if err != nil {
		return err
	}
...
func (v *Validator) countNumFromVotes(qc *maxbftpb.QuorumCert, blockVersion uint32) (int, error) {
	...
	if _, err := VerifyVotes(
		v.ac, len(qc.Votes), qc.Votes, v.validators, blockVersion); err != nil {
		return 0, err
	}
...
func (v *Validator) ValidateJustifyQc(proposal *maxbft.ProposalData) error {
	...
	if proposal.Block.Header.BlockHeight < currentHeight {
		v.logger.Debugf("the block in height[%d] has been committed. currentHeight:%d",
	...
	// todo: check that if the proposal's block version is same with the pre-proposal's or not
	if err := v.ValidateQc(proposal.JustifyQc, proposal.GetBlock().GetHeader().GetBlockVersion()); err != nil {
...
func (v *Validator) validateMinGroupQc(qc *maxbft.QuorumCert, proposal *maxbft.ProposalData) error {
	...
	// verify the votes
	reach, err = VerifyVotes(v.ac, minNodes, qc.Votes, v.validators, proposal.Block.GetHeader().GetBlockVersion())
	if err != nil {
...
func verifyConsensusMsgSign(vote *maxbftpb.VoteData, ac protocol.AccessControlProvider, blockVersion uint32) error {
	if vote == nil {
	...
	return verifyDataSign(data, vote.Signature, ac, blockVersion)
}
...
func validateVoteData(voteData *maxbftpb.VoteData, validators []string,
	...
	if err = verifyDataSign(data, voteData.Signature, ac, blockVersion); err != nil {
...
func VerifyVotes(
	...
	members []string,
	blockVersion uint32) (reachThreshold bool, err error) {
	...
	for i := range votes {
		...
		if err = verifyPool.Submit(func() {
			defer wg.Done()
			if err = validateVoteData(votes[index], members, ac, blockVersion); err != nil {
