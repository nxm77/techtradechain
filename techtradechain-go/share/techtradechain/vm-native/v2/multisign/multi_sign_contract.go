...
func (r *MultiSignRuntime) Vote(txSimContext protocol.TxSimContext,
	parameters map[string][]byte) *commonPb.ContractResult {
...
func (r *MultiSignRuntime) Trig(
	...
	if err != nil {
		...
		return &commonPb.ContractResult{
			...
			Result:        nil,
			Message:       errMsg,
			GasUsed:       invokeContractResult.GasUsed,
			ContractEvent: nil,
		}
	}
...
func (r *MultiSignRuntime) updateUserContract(txSimContext protocol.TxSimContext,
	...
	if multiSignInfo.Method != syscontract.ContractManageFunction_INIT_CONTRACT.String() &&
		multiSignInfo.Method != syscontract.ContractManageFunction_UPGRADE_CONTRACT.String() {
		return nil
	}

	// find user contract name in parameters
	var userContractName string
	for _, param := range multiSignInfo.GetPayload().GetParameters() {
		if param.GetKey() == syscontract.InitContract_CONTRACT_NAME.String() ||
			param.GetKey() == syscontract.UpgradeContract_CONTRACT_NAME.String() {
			userContractName = string(param.GetValue())
			break
		}
	}
	if userContractName == "" {
	...
	// get the contract key in db
	contractDbKey := utils.GetContractDbKey(userContractName)

	// get contract from context
	contractData, err := txSimContext.Get(manageContractName, contractDbKey)
	if err != nil {
		return fmt.Errorf("get contract failed, error: %+v", err)
	}

	// unmarshal the contract
	contract := new(commonPb.Contract)
	err = contract.Unmarshal(contractData)
	if err != nil {
		return fmt.Errorf("unmarshal contract failed, error: %+v", err)
	}
	...
	if multiSignInfo.Method == syscontract.ContractManageFunction_INIT_CONTRACT.String() {
		if block != nil {
			contract.InstallCreateBlockHeight = block.GetHeader().GetBlockHeight()
		}
		contract.InstallCreateTxId = multiSignInfo.GetPayload().GetTxId()
	} else {
		if block != nil {
			contract.UpgradeCreateBlockHeight = block.GetHeader().GetBlockHeight()
		}
		contract.UpgradeCreateTxId = multiSignInfo.GetPayload().GetTxId()
	}

	// marshal the contract
	data, err := contract.Marshal()
	if err != nil {
		return fmt.Errorf("marshal contract failed, error: %+v", err)
	}

	// save the contract to the tx write set
	err = txSimContext.Put(manageContractName, contractDbKey, data)
	if err != nil {
		return fmt.Errorf("put contract failed, error: %+v", err)
	}

	// save the contract by contract address
	if contract.Address != "" {
		addrKey := utils.GetContractDbKey(contract.Address)
		err = txSimContext.Put(manageContractName, addrKey, data)
		if err != nil {
			return err
		}
	}
	r.log.Debugf("update multi sign installed contract information success, contract:%+v", contract)
	return nil
}
...
func (r *MultiSignRuntime) invokeContract(txSimContext protocol.TxSimContext,
	...
	if statusCode == commonPb.TxStatusCode_SUCCESS {
		...
		r.log.Infof("multi sign trig[%s] call contract finished, result: %v", txId, contractResult)
	} else {
	...
	if txSimContext.GetBlockVersion() >= blockVersion237 {
		err = r.updateUserContract(txSimContext, multiSignInfo)
		if err != nil {
			errMsg := fmt.Sprintf(
				"update multiSign installed contract information failed, error: %+v", err)
			return &commonPb.ContractResult{
				Code:          1,
				Result:        nil,
				Message:       errMsg,
