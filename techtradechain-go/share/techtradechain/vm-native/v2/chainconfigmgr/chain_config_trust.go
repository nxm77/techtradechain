...
func (r *ChainTrustRootsRuntime) TrustRootUpdate(txSimContext protocol.TxSimContext, params map[string][]byte) (
	...
	if chainConfig.GetBlockVersion() >= blockVersion233 {
		if useCert(chainConfig.AuthType) || strings.ToLower(chainConfig.AuthType) == protocol.PermissionedWithKey {
			for _, node := range chainConfig.Consensus.Nodes {
				if orgId == node.OrgId {
					err = fmt.Errorf("update trust root cert failed," +
						"you must delete all consensus nodes under the organization first")
					r.log.Error(err)
					return nil, err
				}
			}
		}
	} else if useCert(chainConfig.AuthType) {
		for _, node := range chainConfig.Consensus.Nodes {
			if orgId == node.OrgId {
				err = fmt.Errorf("update trust root cert failed," +
					" you must delete all consensus nodes under the organization first")
...
func (r *ChainTrustRootsRuntime) TrustRootDelete(txSimContext protocol.TxSimContext, params map[string][]byte) (
	...
	// verify has consensus nodes
	err = r.verifyConsensusNodesForDeleteTrustRoots(chainConfig, orgId)
	if err != nil {
		return nil, err
	}
...
func (r *ChainTrustRootsRuntime) verifyConsensusNodesForDeleteTrustRoots(
	chainConfig *configPb.ChainConfig, orgId string) error {
	var err error
	if chainConfig.GetBlockVersion() >= blockVersion233 {
		if useCert(chainConfig.AuthType) || strings.ToLower(chainConfig.AuthType) == protocol.PermissionedWithKey {
			for _, node := range chainConfig.Consensus.Nodes {
				if orgId == node.OrgId {
					err = fmt.Errorf("delete trust root cert failed," +
						" you must delete all consensus nodes under the organization first")
					r.log.Error(err)
					return err
				}
