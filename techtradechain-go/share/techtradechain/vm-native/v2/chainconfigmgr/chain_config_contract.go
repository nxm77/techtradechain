...
func NewChainConfigContract(log protocol.Logger) *ChainConfigContract {
	...
	// handle registration contract methods
	methods := registerChainConfigContractMethods(log)
	chainConfigContract.InitMethod(methods)

	// handle vm methods
	chainConfigContract.registerChainConfigContractMethodsForVM(log)
	return chainConfigContract
}

// registerChainConfigContractMethods  register the contract method except vm module
...
// registerChainConfigContractMethodsForVM  register chain config contract methods for vm
// the VM_SUPPORT_LIST_ADD method can be supported when the blockchain version >= v2.3.7
func (c *ChainConfigContract) registerChainConfigContractMethodsForVM(log protocol.Logger) {
	vmRuntime := &VmRuntime{log: log}
	// do not limit the version
	...
	var startVersion, endVersion uint32 = utils.GetBlockVersion("v2.3.7"), 0
	c.RegisterMethod(syscontract.ChainConfigFunction_VM_SUPPORT_LIST_ADD.String(),
		common.WrapEventResult(vmRuntime.VMSupportListAdd), startVersion, endVersion)

	c.RegisterMethod(syscontract.ChainConfigFunction_VM_SUPPORT_LIST_DEL.String(),
		common.WrapEventResult(vmRuntime.VMSupportListDel), startVersion, endVersion)
}
...
func (r *ChainBlockRuntime) BlockUpdate(txSimContext protocol.TxSimContext, params map[string][]byte) (
	...
	if err != nil {
		...
		r.log.Infof("block update success, param %+v", params)
	}
	return result, err
}
