...
type SDKInterface interface {
	...
	//   - limit: 交易大小限制
	// ```go
	CreatePayload(txId string, txType common.TxType, contractName, method string,
	...
	// **参数说明**
	//   - addrType: 地址类型，0-TechTradeChain; 1-ZXL
	// ```go
	...
	// ### 5.5 多合一订阅
	// **参数说明**
	//   - payload: 消息订阅参数payload
	//   - req: 交易请求
	// ```go
	SubscribeWithTxReq(ctx context.Context, payload *common.Payload, req *common.TxRequest) (<-chan interface{}, error)
	// ```
	...
	//   - withRwSet: 是否返回读写集
	//   - onlyHeader: 若设置为true，将忽略withRwSet选项，仅返回区块头（common.BlockHeader）,若设置为false，将返回common.BlockInfo
	//   - optimizeDetection：优化检查的周期，单位：秒
	//   - switchTimeDiff：切换的时间差，当超过该值没有收到新区块，且有更高的节点时会切换
	// ```go
	SubscribeBlockWithOptimize(ctx context.Context, startBlock, endBlock int64, withRWSet, onlyHeader bool,
		optimizeDetection int, switchTimeDiff int64) (<-chan interface{}, error)
	// ```
	...
	//   - switchTimeDiff：切换的时间差，当超过该值没有收到新区块，且有更高的节点时会切换
	// ```go
	SubscribeContractEventWithOptimize(ctx context.Context, startBlock, endBlock int64, contractName,
		topic string, optimizeDetection int, switchTimeDiff int64) (<-chan interface{}, error)

	// ### 5.10 优化订阅功能，用于非客户端签名生成TxRequest的情况
	// **参数说明**
	//   - txReq: 初始的交易请求
	//   - subscribeSwitchHandler: 当触发切换时，新交易请求相关处理接口
	//   - optimizeDetection：优化检查的周期，单位：秒
	//   - switchTimeDiff：切换的时间差，当超过该值没有收到新区块，且有更高的节点时会切换
	// ```go
	SubscribeWithOptimize(ctx context.Context, txReq *common.TxRequest,
	...
	//   - 输出：已归档的区块高度
	//   - 输出：节点允许的归档高度: 高度必须小于(CurrBlockHeight-UnarchivedHeight)，UnarchivedHeight在techtradechain.yml storage中设置
	...
	// ### 10.2 获取链版本
	// ```go
	GetTechTradeChainServerVersion() (string, error)

	// ### 10.2 获取链版本
	// ```go
	GetTechTradeChainServerVersionCustom(ctx context.Context) (string, error)
	// ```
