...
var (
	EAGIN = errors.New("Resource temporarily unavailable, try again")
)
...
func NewMmapFile(path string, mmSize int) (*MmapFile, error) {
	return OpenMmapFile(path, mmSize, os.O_RDWR|os.O_CREATE, 0644, platform.MAP_SHARED, false)
}
...
func fileFlagToMapPort(flag int) int {
	port := platform.MAP_PROT_READ
	if flag&os.O_WRONLY == os.O_WRONLY {
		port = platform.MAP_PROT_WRITE
	}
	if flag&os.O_RDWR == os.O_RDWR {
		port |= platform.MAP_PROT_READ | platform.MAP_PROT_WRITE
	}
	return port
}
...
func (mf *MmapFile) Seek(offset int64, whence int) (ret int64, err error) {
	...
	case io.SeekEnd:
		s := mf.Size()
		if s < 0 {
			return mf.offset, errors.WithMessage(EAGIN, "MmapFile-Seek")
		}
		offset += s
	}
	if offset < 0 {
