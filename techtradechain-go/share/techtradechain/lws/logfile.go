...
import (
	...
	"techtradechain.com/techtradechain/lws/internal/fbuffer"
...
func openFile(fn string, ft FileType, segmentSize int64) (*file.File, error) {
	switch ft {
...
func newLogFile(fn string, ft FileType, segmentSize int64, bufSize int, mlock bool) (*logfile, error) {
	...
	switch ft {
	case FT_NORMAL, FT_MMAP:
		if bufSize > 0 {
			fb, err = newLogBuffer(f, ft, bufSize, mlock)
			if err != nil {
				return nil, err
			}
			if buf, ok := fb.(*fbuffer.ZeroMmap); ok {
				sync = buf.Sync
			}
		}
	default:
...
func newLogBuffer(f *file.File, ft FileType, bufSize int, mlock bool) (fileBuffer, error) {
	switch ft {
	case FT_MMAP:
		if platform.IsWindows() {
			return fbuffer.NewFixedBuffer(f, bufSize)
		}
		return fbuffer.NewZeroMmap(f, bufSize, platform.MAP_PROT_READ|platform.MAP_PROT_WRITE, platform.MAP_SHARED, mlock)
	default:
		return fbuffer.NewFixedBuffer(f, bufSize)
	}
}
