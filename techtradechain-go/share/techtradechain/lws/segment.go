...
import (
	...
	"techtradechain.com/techtradechain/lws/platform"
)
...
func NewSegmentWriter(s *Segment, opt WriterOptions) (*SegmentWriter, error) {
	if !platform.IsWindows() && opt.Ft == FT_MMAP && opt.Wf == WF_SYNCFLUSH { //mmap sync使用的是munmap，故此同步刷盘下，bufsize设置小即可
		opt.BufferSize = os.Getpagesize()
	}
	sw := &SegmentWriter{
...
func (sw *SegmentWriter) startFlushWorker() {
	if !sw.wf.has(WF_SYNCFLUSH) && sw.wf.has(WF_TIMEDFLUSH) {
...
func (sw *SegmentWriter) Write(t int8, data []byte) (int, error) {
	sw.writeLocker.Lock()
	l, err := sw.write(t, data) //将日志写入文件，如果写入失败，则回退写入游标，以防止用户重试时数据出现错乱
	...
	//如果需要同步写到底层文件中，则将脏数据回写到文件中，写入失败，回退游标，以防止用户重试时数据出现错乱
	if sw.wf.has(WF_SYNCWRITE) {
...
func (sw *SegmentWriter) tryFlush() error {
	if sw.wf.has(WF_SYNCFLUSH) {
		return sw.Flush()
	}
	//如果用户指定了按照写入日志条目累计数进行刷盘，则检测
	if sw.wf.has(WF_QUOTAFLUSH) && sw.acc >= sw.threshold {
		return sw.Flush()
	}
	return nil
}
...
func NewSegmentReader(s *Segment, opt ReadOptions) (*SegmentReader, error) {
	var (
		sr = &SegmentReader{
			SegmentProcessor: newSegmentProcessor(procConfig{
				segmentSize: s.Size,
			...
			s:    s,
			copy: opt.Copy && (opt.BufferSize != 0), //不带缓存的读不进行重复copy
		}
		err error
	)
...
func (sr *SegmentReader) ReadLogByIndex(index uint64) (*LogEntry, error) {
	...
	return sr.readOneEntryFrom(sr.pos[pos], sr.copy), nil
}
