...
func (sg *SegmentGroup) Swap(i, j int) {
	(*sg)[i], (*sg)[j] = (*sg)[j], (*sg)[i]
}

func (sg *SegmentGroup) Less(i, j int) bool {
	return (*sg)[i].ID < (*sg)[j].ID
}

func (sg *SegmentGroup) String() string {
	files := make([]string, 0, sg.Len())
	sg.ForEach(func(i int, s *Segment) bool {
