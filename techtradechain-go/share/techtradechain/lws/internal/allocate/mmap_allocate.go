...
func NewMmapAllocator(f *file.File, offset int64, mmSize int, mapPort, mapFlag int, lock bool) (*MmapAllocator, error) {
...
func mmap(fd int, offset int64, size int, port int, flag int) (_ mmapInfo, err error) {
	...
	area, err = platform.Mmap(fd, offset, size, port, flag)
	if err != nil {
		return
	}
...
func (mal *MmapAllocator) remap(offset int64, size int) error {
	...
	if mal.mmapLock {
		platform.Mlock(mmi.mmArea)
	}

	if mal.mmArea != nil {
		platform.Munmap(mal.mmArea)
	}
...
func (mal *MmapAllocator) Flush() error {
	if mal.mmArea != nil {
		return platform.Msync(mal.mmArea)
	}
	return nil
}
