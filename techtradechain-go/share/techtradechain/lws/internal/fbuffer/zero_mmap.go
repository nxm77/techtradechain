...
import (
	...
	"techtradechain.com/techtradechain/lws/internal/allocate"
...
func NewZeroMmap(f *file.File, mmSize int, mapPort, mapFlag int, lock bool) (*ZeroMmap, error) {
	finfo, err := f.Stat()
	if err != nil {
		return nil, err
	}
...
func (zm *ZeroMmap) nextAt(offset int64, n int) ([]byte, error) {
	...
	if nextEnd > zm.fSize {
		tSize := int64(alignUp(uint64(nextEnd), uint64(OsPageSize)))
		if err := zm.f.Truncate(tSize); err != nil {
			return nil, err
		}
		zm.fSize = tSize
	}
...
func (zm *ZeroMmap) Sync() error {
	...
	// if err := platform.Msync(buf); err != nil {
	// 	return err
	// }
	// platform.Msync(mal.mmArea)
	// zm.allocator.Release()
	return zm.allocator.Flush()
	// zm.waitSync = area{}
	// return nil
}
