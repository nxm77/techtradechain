...
func (pool *batchTxPool) validateBatchSign(batch *txpoolPb.TxBatch) error {
	...
	if strings.ToLower(pool.chainConf.ChainConfig().AuthType) == protocol.PermissionedWithCert {
		...
		var result bool
		result, err = pool.ac.VerifyMsgPrincipal(principal, pool.chainConf.ChainConfig().GetBlockVersion())
		if err != nil {
...
func (pool *batchTxPool) validateTx(tx *commonPb.Transaction, verifyMods ...TxVerifyMod) (*memTx, error) {
	...
	for _, verifyMod := range verifyMods {
		...
		case VerifyTxAuth:
			if err = utils.VerifyTxWithoutPayload(tx, pool.chainId, pool.ac,
				pool.chainConf.ChainConfig().GetBlockVersion()); err != nil {
