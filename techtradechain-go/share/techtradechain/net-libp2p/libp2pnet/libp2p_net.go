...
import (
	...
	libP2pPubSub "techtradechain.com/techtradechain/libp2p-pubsub"
	"techtradechain.com/techtradechain/net-common/common"
	//"techtradechain.com/techtradechain/net-common/common/pool"
	"techtradechain.com/techtradechain/net-common/common/priorityblocker"
	"techtradechain.com/techtradechain/net-common/utils"
	"techtradechain.com/techtradechain/net-libp2p/datapackage"
...
const (
	...
	DefaultLibp2pServiceTag = "techtradechain-libp2p-net"
...
// MsgPID is the protocol.ID of techtradechain net msg.
const MsgPID = protocol.ID("/TechTradeChainNetMsg/1.0.0/")
...
const refreshCertIdTickerTime = time.Duration(time.Second * 60)
...
func NewLibP2pNet(log api.Logger) (*LibP2pNet, error) {
	...
	net := &LibP2pNet{
		...
		prepare: &LibP2pNetPrepare{
			...
			httpTunnelTargetAddress: make(map[string]string),
	...
	net.router = newRouterController(log, net)
...
func (ln *LibP2pNet) InitPubSub(chainId string, maxMessageSize int) error {
	...
	go ln.checkCertId(chainId)
	ln.router.startMsgRouter(chainId)
	return nil
}

func (ln *LibP2pNet) StopPubSub(chainId string) {
	err := ln.CancelSubscribeWithChainId(chainId, TOPIC_CERTID)
	if err != nil {
		ln.log.Warn("[Net] CancelSubscribeWithChainId failed ", err.Error())
	}
	ln.router.stopMsgRouter(chainId)
	ps, ok := ln.pubSubs.LoadAndDelete(chainId)
	if !ok {
		return
	}
	pubSub, ok := ps.(*LibP2pPubSub)
	if !ok {
		return
	}
	pubSub.Stop()
...
func (ln *LibP2pNet) topicSubLoop(
	...
	for {
		...
		if err != nil {
			if err.Error() == "subscription cancelled" || err.Error() == "context canceled" {
				ln.log.Warn("[Net] ", err)
				break
			}
			//logger
			ln.log.Infof("[Net] subscribe next failed, %s", err.Error())
		}
		if message == nil {
			return
		}
		...
		go func(msg *libP2pPubSub.Message) {
			...
			if err = handler(message.GetFrom().Pretty(), bytes); err != nil {
				if err == ErrorNotConnected {
					ln.log.Debugf("[Net] call subscribe handler failed, %s ", err)
				} else {
...
func (ln *LibP2pNet) SendMsg(chainId string, node string, msgFlag string, data []byte) error {
	...
	if !isConnected { // is peer connected
		...
		//	return ErrorNotConnected // node not connected
		//}
		if ln.router.maxStepCount <= 0 {
			return ErrorNotConnected
		}
		return ln.router.sendByMsgRouter(chainId, pid, msgFlag, data)
...
func (ln *LibP2pNet) reloadChainPubSubWhiteListLoop(chainId string, ps *LibP2pPubSub) {
	if ln.startUp {
		...
		for {
			...
			case <-ps.ctx.Done():
				ln.reloadChainPubSubWhiteListSignalChanMap.Delete(chainId)
				return
			}
		}
	}
...
func (ln *LibP2pNet) ChainNodesInfo(chainId string) ([]*api.ChainNodeInfo, error) {
	...
	for peerId := range peerIds {
		...
		if err != nil {
			ln.log.Error("Decode(%v) err,%v", peerId, err)
			continue
		}
		addrs := make([]string, 0)
...
func (ln *LibP2pNet) DeleteAC(chainId string) {
	ln.libP2pHost.memberStatusValidator.DeleteAC(chainId)
}
