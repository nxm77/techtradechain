...
// GetRandomConnExcept return a random peer , high-level is first.
func (cm *PeerConnManager) GetRandomConnExcept(ignore map[string]struct{}) (peer.ID, error) {
	rand.Seed(time.Now().UnixNano())

	cm.cmLock.RLock()
	defer cm.cmLock.RUnlock()

	// 收集所有不在ignore中的high-level节点
	var availableHighPeers []peer.ID
	for _, conn := range cm.highLevelConn {
		if _, ignored := ignore[conn.pid.Pretty()]; !ignored {
			availableHighPeers = append(availableHighPeers, conn.pid)
		}
	}
	// 如果有可用的high-level节点，从中随机选择一个
	if len(availableHighPeers) > 0 {
		randomIndex := rand.Intn(len(availableHighPeers))
		return availableHighPeers[randomIndex], nil
	}
	// 收集所有不在ignore中的low-level节点
	var availableLowPeers []peer.ID
	for _, conn := range cm.lowLevelConn {
		if _, ignored := ignore[conn.pid.Pretty()]; !ignored {
			availableLowPeers = append(availableLowPeers, conn.pid)
		}
	}
	// 如果有可用的low-level节点，从中随机选择一个
	if len(availableLowPeers) > 0 {
		randomIndex := rand.Intn(len(availableLowPeers))
		return availableLowPeers[randomIndex], nil
	}
