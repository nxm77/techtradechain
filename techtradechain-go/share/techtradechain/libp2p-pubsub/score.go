...
func (ps *peerScore) ValidateMessage(msg *Message) {
	...
	// the libp2ppubsub subsystem is beginning validation; create a record to track time in
...
func (ps *peerScore) DeliverMessage(msg *Message) {
	...
	if drec.status != deliveryUnknown {
		log.Warnf("unexpected delivery trace: message from %s was first seen %s ago and has delivery status %d", msg.ReceivedFrom, time.Since(drec.firstSeen), drec.status)
		return
	}
...
func (ps *peerScore) RejectMessage(msg *Message, reason string) {
	...
	if drec.status != deliveryUnknown {
		log.Warnf("unexpected rejection trace: message from %s was first seen %s ago and has delivery status %d", msg.ReceivedFrom, time.Since(drec.firstSeen), drec.status)
		return
	}
