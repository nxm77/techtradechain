...
func (pool *txPoolImpl) AddTx(tx *commonPb.Transaction, source protocol.TxSource) error {
	...
	if pool.isFull(tx) {
		if source == protocol.RPC && TxPoolConfig.EnableRelayWhenFull {
	...
	if source == protocol.RPC {
		...
		if localconf.TechTradeChainConfig.DebugConfig.IsNoBroadcastTx {
...
func (pool *txPoolImpl) dumpTxs() (err error) {
	dumpPath := path.Join(localconf.TechTradeChainConfig.GetStorePath(), pool.chainId, dumpDir)
