...
var (
	...
	// TechTradeChainConfig is the CMConfig instance for global.
	TechTradeChainConfig = &CMConfig{StorageConfig: map[string]interface{}{}}
)
...
func InitLocalConfig(cmd *cobra.Command) error {
	...
	// 4. set global techtradechain config
	TechTradeChainConfig = cmConfig
...
func compareThenSetNewCMConfigWithCurrent(newCmConfig *CMConfig) {
	...
	for idx := range TechTradeChainConfig.BlockChainConfig {
		existedChain[TechTradeChainConfig.BlockChainConfig[idx].ChainId] = struct{}{}
	}
	...
	if len(newChainIdsFound) > 0 {
		...
		if newCmConfig.NodeConfig.NodeId == "" {
			newCmConfig.SetNodeId(TechTradeChainConfig.NodeConfig.NodeId)
		}
		TechTradeChainConfig = newCmConfig
...
func compareThenDeleteCMConfigWithCurrent(newCmConfig *CMConfig) {
	if len(TechTradeChainConfig.BlockChainConfig) == 1 {
		return
	}
	...
	for idx := range TechTradeChainConfig.BlockChainConfig {
		oldChainId := TechTradeChainConfig.BlockChainConfig[idx].ChainId
...
func initCmConfigForLogOnly() (*CMConfig, error) {
	...
	logYmlFile := TechTradeChainConfig.LogConfig.ConfigFile
...
func UpdateDebugConfig(pairs []*config.ConfigKeyValue) error {
	value := reflect.ValueOf(&TechTradeChainConfig.DebugConfig)
	elem := value.Elem()
