...
import (
	...
	"techtradechain.com/techtradechain/net-liquid/core/protocol"
	"techtradechain.com/techtradechain/net-liquid/core/util"
	"techtradechain.com/techtradechain/net-liquid/discovery/protocoldiscovery/pb"
	"techtradechain.com/techtradechain/net-liquid/logger"
...
// NewProtocolBasedDiscovery .
// @Description:
// @param host
// @param opts
// @return *ProtocolBasedDiscovery
// @return error
...
func (d *ProtocolBasedDiscovery) createProtocolMsgHandler(
	...
	return func(senderPID peer.ID, msgPayload []byte) {
		...
		case pb.DiscoveryMsg_Quit:
			// announce type msg
			if len(msg.PInfos) == 0 {
				d.logger.Warnf("nil or empty pid. (msg type: %s)", pb.DiscoveryMsg_Quit.String())
				return
			}
			// load peer.ID
			newSupportPid := peer.ID(msg.PInfos[0].Pid)
			// if i have the addr info of peer, add this protocol to the list that peer supported
			if d.host.PeerStore().GetFirstAddr(newSupportPid) != nil {
				d.host.PeerStore().DeleteProtocol(newSupportPid, protocol)
			}
...
// @param serviceName
// @return error
func (d *ProtocolBasedDiscovery) Quit(_ context.Context, serviceName string) error {
	serviceName = strings.TrimSpace(serviceName)
	// create protocol ID
	protoID := d.createProtocolIDWithServiceName(serviceName)
	_, ok := d.svcMap.Load(serviceName)
	if ok {
		// unregister discovery protocol to host
		err := d.host.UnregisterMsgPayloadHandler(protoID)
		if err != nil {
			return err
		}
		d.svcMap.Delete(serviceName)
	}
	...
	for i := range allPeers {
		...
		go func() {
			e := d.host.SendMsg(protoID, rPid, msgBytes)
			if e != nil {
				d.logger.Warnf("send discovery quit msg failed, %s", err.Error())
			}
		}()
	}
	return nil
...
// @receiver d
// @param serviceName
func (d *ProtocolBasedDiscovery) StopFindPeers(serviceName string) {
	serviceName = strings.TrimSpace(serviceName)
	fChain, ok := d.findingMap.LoadAndDelete(serviceName)
	if !ok {
		return
	}
	close(fChain.(chan ma.Multiaddr))
}
