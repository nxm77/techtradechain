...
import (
	...
	cmTlsS "techtradechain.com/techtradechain/net-common/cmtlssupport"
	"techtradechain.com/techtradechain/net-common/common"
	"techtradechain.com/techtradechain/net-common/common/priorityblocker"
	"techtradechain.com/techtradechain/net-common/utils"
	"techtradechain.com/techtradechain/net-liquid/core/broadcast"
	"techtradechain.com/techtradechain/net-liquid/core/discovery"
	"techtradechain.com/techtradechain/net-liquid/core/handler"
	...
	"techtradechain.com/techtradechain/net-liquid/discovery/protocoldiscovery"
	"techtradechain.com/techtradechain/net-liquid/exMultiaddr"
	"techtradechain.com/techtradechain/net-liquid/holepunch"
	lHost "techtradechain.com/techtradechain/net-liquid/host"
	"techtradechain.com/techtradechain/net-liquid/host/quic"
	"techtradechain.com/techtradechain/net-liquid/host/tcp"
	"techtradechain.com/techtradechain/net-liquid/pubsub"
	"techtradechain.com/techtradechain/net-liquid/relay"
	"techtradechain.com/techtradechain/net-liquid/stun"
	"techtradechain.com/techtradechain/net-liquid/tlssupport"
...
// StopPubSub stop the pubsub by chainId
// StopPubSub .
// @Description:
// @receiver l
// @param chainId
func (l *LiquidNet) StopPubSub(chainId string) {
	_ = l.cleanDiscovery(chainId)
	ps, ok := l.psMap.LoadAndDelete(chainId)
	if !ok {
		return
	}
	pubSub, ok := ps.(broadcast.PubSub)
	if !ok {
		return
	}
	_ = pubSub.CleanHost()
...
// createMsgPayloadHandler .
// @Description:
// @receiver l
// @param handler
// @return handler.MsgPayloadHandler
...
// resetChainPubSubBlackPeerWithPid .
// @Description:
// @receiver l
// @param pidStr
...
// queryAndStoreDerivedInfoInCertValidator .
// @Description:
// @receiver l
// @param peerIdStr
...
func (l *LiquidNet) listenFindingChanTask(c <-chan ma.Multiaddr) {
	for {
		...
		case ai := <-c:
			if ai == nil {
				log.Info("[LiquidNet] close the find peer chain...")
				return
			}
			...
			_, err := l.host.Dial(ai)
			if err != nil {
				log.Infof("[LiquidNet] [Discovery] Dial(pid: %s, addr: %s)err:%v", pid, addr.String(),err)
			}
		}
	}
}
...
// @param chainId
// @return error
func (l *LiquidNet) cleanDiscovery(chainId string) error {
	err := l.discoveryService.Quit(l.context, chainId)
	if err != nil {
		return err
	}
	log.Infof("[LiquidNet] quit the chain service. (chain-id: %s)", chainId)
	l.discoveryService.StopFindPeers(chainId)
	log.Infof("[LiquidNet] the find peers has been stoped... (chain-id: %s)", chainId)
	return nil
}
...
func (l *LiquidNet) DeleteAC(chainId string) {
	l.memberStatusValidator.DeleteAC(chainId)
}
