...
import (
	...
	"techtradechain.com/techtradechain/net-liquid/core/mgr"
	"techtradechain.com/techtradechain/net-liquid/core/network"
	"techtradechain.com/techtradechain/net-liquid/core/peer"
	"techtradechain.com/techtradechain/net-liquid/core/types"
	"techtradechain.com/techtradechain/net-liquid/core/util"
...
type LevelConnManager struct {
	...
	cmLock sync.RWMutex
	// maximum number of peers allowed to connection
	maxPeerCountAllowed int
	// maximum number of connections allowed between peers
	maxConnCountEachPeerAllowed int
	// the connection elimination strategy used
	// when the number of connected peers reaches the upper limit
	strategy           EliminationStrategy
	highLevelPeersLock sync.RWMutex
	// record the collection of high-level peers
	highLevelPeers *types.PeerIdSet
	// a list of objects that record connections to higher-level peers
	highLevelConn []*peerConnections
	// a list of objects that record connections to low-level peers
...
// getHighLevelConnections get high level connections
...
// getLowLevelConnections get low level connections
...
// eliminateConnections enforce connection elimination policies
...
// closeLowLevelConnWithIdx closes the connection object at the specified index in the lower-level connection list
...
// closeHighLevelConnWithIdx closes the connection object at the specified index in the high-level connection list
...
// closeLowLevelConnRandom randomly eliminate the connection object corresponding to a peer from
// the low-level connection list
...
// closeHighLevelConnRandom randomly eliminate the connection object corresponding to a peer from
// the high-level connection list
...
// eliminateConnectionsRandom execute Random connection elimination strategy
...
// closeLowLevelConnFirst eliminate the connection object corresponding to the first peer from
// the low-level connection list
...
// closeHighLevelConnFirst eliminate the connection object corresponding to the first peer from
// the high-level connection list
...
// eliminateConnectionsFIFO execute FIFO connection elimination strategy
...
// closeLowLevelConnLast eliminate the connection object corresponding to the last peer from
// the low-level connection list
...
// closeHighLevelConnLast eliminate the connection object corresponding to the last peer from
// the low-level connection list
...
// eliminateConnectionsLIFO execute LIFO connection elimination strategy
...
// if the maximum number of connections is reached, the connection elimination strategy will be executed
...
func (cm *LevelConnManager) IsAllowed(pid peer.ID) bool {
	...
	isHighLevel := cm.IsHighLevel(pid)
	// When the strategy is LIFO, priority should be given to high-level connections.
	if cm.strategy == LIFO {
		// If it is a high-level connection,
		// we only need to determine whether the number of high-level connections reaches the upper limit.
		if isHighLevel {
			return len(cm.highLevelConn) < cm.maxPeerCountAllowed
		}
		// If it is a LowLevel connection,
		// we need to determine whether the number of all connections reaches the upper limit.
		return len(cm.highLevelConn)+len(cm.lowLevelConn) < cm.maxPeerCountAllowed
	}

	// If the strategy is not LIFO and the number of connections reaches the upper limit,
	// we should directly accept the high-level connections, and one other connection will be eliminated.
	// For low-level connections, we need to prioritize ensuring that high-level connections will not be eliminated.
	if !isHighLevel {
		return len(cm.highLevelConn) < cm.maxPeerCountAllowed
	}
	return true
