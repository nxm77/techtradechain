...
func (c *conn) compareDirection(conn net.Conn, remoteAddr ma.Multiaddr) network.Direction {
	...
	// receive pid
	lenPid := 46
	remotePid := make([]byte, lenPid)
	readed := 0
	readLen := 0
	for readed < lenPid {
		readLen, err = conn.Read(remotePid[readed:])
		if err != nil {
			_ = conn.Close()
			return c.Direction()
		}
	...
	compare := strings.Compare(string(c.lPID), string(remotePid))
	...
	c.nw.logger.Debugf("compareDirection (remote pid:%v ,Direction:%v,dir:%v)", remotePid, c.Direction(), dir)
	return dir
}
...
func newConn(ctx context.Context, nw *tcpNetwork, c net.Conn,
	...
	if needCompare && os.Getenv("liquid_enable_holepunch") == "1" {
