...
import (
	...
	"techtradechain.com/techtradechain/vm-gasm/v2/gasm-go/wasm/leb128"
)
...
func (m *Module) BuildIndexSpacesUsingOldNativeFunction(externModules map[string]*Module, nativeFunctions []VirtualMachineFunction) error {
	...
	brsSlice := make([]map[uint64]*NativeFunctionBlock, 0)

	for _, function := range nativeFunctions {
		if nativeFunction, ok := function.(*NativeFunction); ok {
			brsSlice = append(brsSlice, nativeFunction.Blocks)
		}
	}

	if err := m.buildFunctionIndexSpaceUsingOldNativeFunction(brsSlice); err != nil {
		return fmt.Errorf("build function index space: %w", err)
	}
	if err := m.buildTableIndexSpace(); err != nil {
		return fmt.Errorf("build table index space: %w", err)
...
func (m *Module) resolveImports(externModules map[string]*Module) error {
	for _, is := range m.SecImports {
		...
		if is.Desc.Kind != es.Desc.Kind {
			return fmt.Errorf("%s type mismatch on export: got %#x but want %#x", is.Name, es.Desc.Kind, is.Desc.Kind)
		}
		switch is.Desc.Kind {
		case 0x00: // function
			if err := m.applyFunctionImport(is, em, es, -1); err != nil {
				return fmt.Errorf("%s applyFunctionImport failed: %w", is.Name, err)
			}
		case 0x01: // table
			if err := m.applyTableImport(em, es); err != nil {
				return fmt.Errorf("%s applyTableImport failed: %w", is.Name, err)
			}
		case 0x02: // mem
			if err := m.applyMemoryImport(em, es); err != nil {
				return fmt.Errorf("%s applyMemoryImport: %w", is.Name, err)
			}
		case 0x03: // global
...
func insert(array []VirtualMachineFunction, element VirtualMachineFunction, i int) []VirtualMachineFunction {
	return append(array[:i], append([]VirtualMachineFunction{element}, array[i:]...)...)
}

func (m *Module) applyFunctionImport(is *ImportSegment, em *Module, es *ExportSegment, index int) error {
	...
	if index >= 0 {
		m.IndexSpace.Function = insert(m.IndexSpace.Function, f, index)
	} else {
...
func (m *Module) buildFunctionIndexSpaceUsingOldNativeFunction(brsSlice []map[uint64]*NativeFunctionBlock) error {
	for codeIndex, typeIndex := range m.SecFunctions {
		f := &NativeFunction{
			Signature: m.SecTypes[typeIndex],
			Body:      m.SecCodes[codeIndex].Body,
			NumLocal:  m.SecCodes[codeIndex].NumLocals,
			Blocks:    brsSlice[codeIndex],
		}
