...
var hostMethodGasMap map[string]uint64

func init() {
	hostMethodGasMap := make(map[string]int64, 64)
	hostMethodGasMap["techtradechain.com/techtradechain-go/gasm/wasi.fd_write.func1"] = 1000
	hostMethodGasMap["techtradechain.com/techtradechain-go/gasm/examples.abc"] = 100000
}
...
func NewVM(module *Module, gasUsed uint64, gasLimit uint64, timeLimit uint64) (*VirtualMachine, error) {
	...
	vm.StartTime = uint64(time.Now().UnixNano())
	vm.Gas = gasUsed
	vm.GasMap = hostMethodGasMap
...
func (vm *VirtualMachine) FetchInt32() int32 {
	m := 5
	if len(vm.ActiveContext.Function.Body)-int(vm.ActiveContext.PC) < 5 {
		m = len(vm.ActiveContext.Function.Body) - int(vm.ActiveContext.PC)
	}

	ret, num, err := leb128.DecodeInt32B(vm.ActiveContext.Function.Body[vm.ActiveContext.PC : vm.ActiveContext.PC+uint64(m)])
	if err != nil {
		panic(err)
	}
...
func (vm *VirtualMachine) FetchUint32() uint32 {
	...
	ret, num, err := leb128.DecodeUint32B(vm.ActiveContext.Function.Body[vm.ActiveContext.PC : vm.ActiveContext.PC+uint64(m)])
	if err != nil {
		panic(err)
	}
	vm.ActiveContext.PC += num - 1
...
func (vm *VirtualMachine) CalculateTime() {
	vm.EndTime = uint64(time.Now().UnixNano())
	if vm.EndTime-vm.StartTime > vm.TimeLimit {
		panic("timeout")
	}
}
