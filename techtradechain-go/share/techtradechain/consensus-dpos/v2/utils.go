...
import (
	...
	pbdpos "techtradechain.com/techtradechain/pb-go/v2/consensus/dpos"
...
// getLatestEpochInfo get information on the last epoch
...
// getNodeIDsFromValidators query the corresponding node id of the validator from the ledger,
...
// getChainConfig query chain configuration from ledger
...
// GetValidatorList get validator list, used by techtradechain-go
// Only used for dpos consensus,
// returns an error when configuring other consensus algorithms
// nolint: unused
func GetValidatorList(chainConfig *configPb.ChainConfig, store protocol.BlockchainStore) (
...
// getValidatorListFromLedger get validator list from ledger
func getValidatorListFromLedger(store protocol.BlockchainStore) (validators []string, err error) {
	epoch, err := getLatestEpochInfo(store)
	if err != nil {
		return nil, err
	}
	// query the corresponding node id of the validator from the ledger,
	// and return the list of node ids
	nodeIDs, err := getNodeIDsFromValidators(store, epoch.ProposerVector)
	if err != nil {
		return nil, err
	}
	return nodeIDs, nil
