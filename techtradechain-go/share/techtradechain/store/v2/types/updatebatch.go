...
// NewSyncPoolUpdateBatch init a sync.Pool to spools then return it
// @Description:
// @param batchPool
// @return *sync.Pool
func NewSyncPoolUpdateBatch() *sync.Pool {
	batchPool := &sync.Pool{}
	batchPool.New = func() interface{} {
		return NewUpdateBatch()
	}
	return batchPool
}
...
// BatchToIndexPairs convert updateBatch kvs to index pairs
// @Description:
// @param blkBatch
// @return []*tbf.IndexPair
func BatchToIndexPairs(batch *UpdateBatch) []*tbf.IndexPair {
	if batch == nil {
		return nil
	}
	pairs := make([]*tbf.IndexPair, 0, batch.Len())
	for _, key := range batch.Keys() {
		v, _ := batch.Get([]byte(key))
		pairs = append(pairs, &tbf.IndexPair{
			Key:   []byte(key),
			Value: v,
		})
	}
...
// IndexPairsToBatch convert index pairs to updateBatch kvs
// @Description:
// @param blkBatch
// @return []*tbf.IndexPair
func IndexPairsToBatch(kvPairs []*tbf.IndexPair, batch protocol.StoreBatcher,
	logger protocol.Logger) protocol.StoreBatcher {
	if kvPairs == nil {
		return nil
	}
