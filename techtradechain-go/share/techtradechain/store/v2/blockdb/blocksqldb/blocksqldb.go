...
// BlockSqlDB provider an implementation of `blockdb.BlockDB`
...
// IsArchivedPivotKeyZeroStatus  not implement
func (b *BlockSqlDB) IsArchivedPivotKeyZeroStatus() bool {
	//TODO implement me
	panic("implement me")
}
...
func (b *BlockSqlDB) GetBlockHeaderByHeight(height uint64) (*commonPb.BlockHeader, error) {
	...
	blockInfo, err := b.getBlockInfoBySql(sql, height)
	if err != nil {
		return nil, err
	}
...
//	@receiver db
//	@param dbName
func (b *BlockSqlDB) initDb(dbName string) {
	_, err := b.db.CreateDatabaseIfNotExist(dbName)
	if err != nil {
		panic("init state sql db fail")
	}

	err = b.db.CreateTableIfNotExist(&BlockInfo{})
	if err != nil {
		panic("init state sql db table `block_infos` fail" + err.Error())
	}
	err = b.db.CreateTableIfNotExist(&TxInfo{})
	if err != nil {
