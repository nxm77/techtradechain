...
func (s *StateKvDB) DirectFlushDB(keyMap map[string][]byte) error {
	chainConfig, err := s.GetChainConfig()
	if err != nil {
		return err
	}
	...
	// <= v2.3.3 || == v3.0.0
	if blockVersion <= v233 || blockVersion == v300 {
		for k, v := range keyMap {
			if err := s.dbHandle.Put([]byte(k), v); err != nil {
				s.logger.Errorf("FlushDB error: %v", err)
				return err
			}
		}
		return nil
	...
	return s.dbHandle.WriteBatch(batch, true)
...
func (s *StateKvDB) GetLastSavepoint() (uint64, error) {
	bytes, err := s.get([]byte(stateDBSavepointKey))
	if err != nil {
		return 0, err
	} else if bytes == nil {
		return 0, nil
...
func (s *StateKvDB) writeBatch(blockHeight uint64, batch protocol.StoreBatcher) error {
	//经测试leveldb可以进行异步写，即高度写入代表此高度的kv值也会落盘
	isLevelDB := s.dbHandle.GetDbType() == leveldbprovider.DbType_Leveldb
	start := time.Now()

	//remove savePoint from batch
	lastKey := []byte(stateDBSavepointKey)
	...
	for i := 0; i < len(batches); i++ {
		go func(index int) {
			defer wg.Done()
			if err = s.dbHandle.WriteBatch(batches[index], !isLevelDB); err != nil {
	...
	// write savePoint
	if err = s.dbHandle.Put(lastKey, value); err != nil {
		panic(fmt.Sprintf("Error writing state db: %s", err))
	}
	...
	//err := s.dbHandle.WriteBatch(batch, true)
	//s.RWMutex.Unlock()
	//if err != nil {
