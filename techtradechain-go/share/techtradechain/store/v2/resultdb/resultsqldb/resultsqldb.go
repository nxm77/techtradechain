...
// ResultSqlDB provider an implementation of `history.HistoryDB`
// @Description:
...
// @receiver db
// @param dbName
func (h *ResultSqlDB) initDb(dbName string) {
	h.logger.Debugf("create result database %s to save transaction receipt", dbName)
	_, err := h.db.CreateDatabaseIfNotExist(dbName)
	if err != nil {
		h.logger.Panicf("init state sql db fail,error:%s", err)
	}
	h.logger.Debug("create table result_infos")
	err = h.db.CreateTableIfNotExist(&ResultInfo{})
	if err != nil {
		h.logger.Panicf("init state sql db table `state_history_infos` fail, error:%s", err)
	}
	err = h.db.CreateTableIfNotExist(&types.SavePoint{})
	if err != nil {
		h.logger.Panicf("init state sql db table `save_points` fail,error:%s", err)
	}
	_, err = h.db.Save(&types.SavePoint{})
	if err != nil {
		h.logger.Panicf("insert new SavePoint to table get an error:%s", err)
	}
}
