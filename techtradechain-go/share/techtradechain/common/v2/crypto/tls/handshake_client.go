...
func (c *Conn) loadSession(hello *clientHelloMsg) (cacheKey string,
	...
	if !c.config.InsecureSkipVerify {
		...
		//如果设置了环境变量CM_CERT_NO_EXPIRE，那么就不检查证书的有效期，证书永久可用
		if c.config.time().After(serverCert.NotAfter) && os.Getenv("CM_CERT_NO_EXPIRE") != "true" {
...
func (c *Conn) verifyServerCertificate(certificates [][]byte) error {
	...
	for i, asn1Data := range certificates {
		cert, err := cmx509.ParseCertificate(asn1Data)
		if err != nil {
			c.sendAlert(alertBadCertificate)
	...
	switch x509Cert0.PublicKey.(type) {
...
var (
	tls11SignatureSchemes      = []SignatureScheme{ECDSAWithP256AndSHA256, ECDSAWithP384AndSHA384, ECDSAWithP521AndSHA512, SM2WithSM3, PKCS1WithSHA256, PKCS1WithSHA384, PKCS1WithSHA512, PKCS1WithSHA1}
