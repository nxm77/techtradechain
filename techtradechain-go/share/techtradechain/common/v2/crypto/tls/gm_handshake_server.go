...
func (hs *serverHandshakeStateGM) readClientHello() (isResume bool, err error) {
	...
	// GMT0024
	if c.config.GMSupport.EncCertEnable && len(hs.cert) < 2 {
		c.sendAlert(alertInternalError)
...
func (hs *serverHandshakeStateGM) doFullHandshake() error {
	...
	if c.config.GMSupport.EncCertEnable {
		skx, err = keyAgreement.generateServerKeyExchange(c.config, &hs.cert[0], &hs.cert[1], hs.clientHello, hs.hello)
	} else {
		skx, err = keyAgreement.generateServerKeyExchange(c.config, &hs.cert[0], &hs.cert[0], hs.clientHello, hs.hello)
	}
	if err != nil {
		c.sendAlert(alertHandshakeFailure)
		return err
	...
	if c.config.GMSupport.EncCertEnable {
		...
		preMasterSecret, err = keyAgreement.processClientKeyExchange(c.config, &hs.cert[0], ckx, c.vers)
	}
	if err != nil {
		c.sendAlert(alertHandshakeFailure)
		return err
	...
	if len(c.peerCertificates) > 0 {
		...
		if err = verifyHandshakeSignature(sigType, pub, hashFunc, digest, certVerify.signature); err != nil {
			c.sendAlert(alertBadCertificate)
...
func (hs *serverHandshakeStateGM) processCertsFromClient(certificates [][]byte) (crypto.PublicKey, error) {
	...
	certs := make([]*cmx509.Certificate, len(certificates))
	var err error
	for i, asn1Data := range certificates {
		if certs[i], err = cmx509.ParseCertificate(asn1Data); err != nil {
			c.sendAlert(alertBadCertificate)
	...
	if c.config.ClientAuth >= VerifyClientCertIfGiven && len(certs) > 0 {
		opts := cmx509.VerifyOptions{
			Roots:         c.config.ClientCAs,
			CurrentTime:   c.config.time(),
			Intermediates: cmx509.NewCertPool(),
			KeyUsages:     []gox509.ExtKeyUsage{gox509.ExtKeyUsageClientAuth},
		}
		...
		if err != nil {
			c.sendAlert(alertBadCertificate)
			return nil, errors.New("tls: failed to verify client certificate: " + err.Error())
		}
	...
	x509Cert0, _ := cmx509.TechTradeChainCertToX509Cert(certs[0])
	var pub crypto.PublicKey
	switch key := x509Cert0.PublicKey.(type) {
	case *ecdsa.PublicKey, *rsa.PublicKey, ed25519.PublicKey, *sm2.PublicKey:
		pub = key
	default:
		c.sendAlert(alertUnsupportedCertificate)
		return nil, fmt.Errorf("tls: client certificate contains an unsupported public key of type %T", certs[0].PublicKey)
	}
...
func (hs *serverHandshakeStateGM) clientHelloInfo() *ClientHelloInfo {
	...
	// supportedVersions only include VersionGMSSL
