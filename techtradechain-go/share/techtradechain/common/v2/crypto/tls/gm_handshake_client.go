...
func (hs *clientHandshakeStateGM) doFullHandshake() error {
	...
	if c.handshakes == 0 {
		...
		for i, asn1Data := range certMsg.certificates {
			...
			_, ok := cert.PublicKey.ToStandardKey().(*sm2.PublicKey)
			if !ok {
			...
			if c.config.GMSupport.EncCertEnable {
				...
				switch i {
				case 0:
					if cert.KeyUsage == 0 || (cert.KeyUsage&(gox509.KeyUsageDigitalSignature|cert.KeyUsage&gox509.KeyUsageContentCommitment)) == 0 {
						c.sendAlert(alertInsufficientSecurity)
				...
				case 1:
					if cert.KeyUsage == 0 || (cert.KeyUsage&(gox509.KeyUsageDataEncipherment|gox509.KeyUsageKeyEncipherment|gox509.KeyUsageKeyAgreement)) == 0 {
						c.sendAlert(alertInsufficientSecurity)
		...
		if !c.config.InsecureSkipVerify {
			...
			/**
			TASSL certificate chain:
			1. signCert + rootCA + encCert
			**/
		...
		pubKey := certs[0].PublicKey.ToStandardKey()
		switch pubKey.(type) {
...
findCert:
	for i, chain := range c.config.Certificates {
		...
		for j, cert := range chain.Certificate {
			...
			if j != 0 || x509Cert == nil {
				var err error
				cmCert, err := x509.ParseCertificate(cert)
				if err != nil {
					c.sendAlert(alertInternalError)
					return nil, errors.New("tls: failed to parse client certificate #" + strconv.Itoa(i) + ": " + err.Error())
				}
				x509Cert, err = x509.TechTradeChainCertToX509Cert(cmCert)
				if err != nil {
					c.sendAlert(alertInternalError)
					return nil, errors.New("tls: failed to parse client certificate #" + strconv.Itoa(i) + ": " + err.Error())
				}
			...
			if x509Cert.PublicKeyAlgorithm == gox509.ECDSA {
			...
			if len(certReq.certificateAuthorities) == 0 {
				...
				if chainToSend.Certificate == nil {
					chainToSend.Certificate = chain.Certificate
