...
import (
	...
	cmcrypto "techtradechain.com/techtradechain/common/v2/crypto"
...
func verifyHandshakeSignature(sigType uint8, pubkey crypto.PublicKey, hashFunc crypto.Hash, signed, sig []byte) error {
	if cmPubkey, ok := pubkey.(cmcrypto.PublicKey); ok {
		pubkey = cmPubkey.ToStandardKey()
	}
	switch sigType {
	case signatureECDSA:
...
func signedMessage(sigHash crypto.Hash, context string, transcript hash.Hash) []byte {
	if sigHash == directSigning || sigHash == cmcrypto.SM3 {
		b := &bytes.Buffer{}
...
func legacyTypeAndHashFromPublicKey(pub crypto.PublicKey) (sigType uint8, hash crypto.Hash, err error) {
	...
	case *sm2.PublicKey:
		return signatureSM2, cmcrypto.SM3, nil
...
func unsupportedCertificateError(cert *Certificate) error {
	...
	case *sm2.PublicKey:
		if pub.Curve != sm2.P256Sm2() {
			return fmt.Errorf("tls: require curve P256Sm2, get curve %s", pub.Curve.Params().Name)
		}
	case *rsa.PublicKey:
