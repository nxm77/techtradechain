...
func argGetter(argSpec []Argument, args []interface{}, ptr bool) (func(int) interface{}, error) {
	if len(args) == 1 {
		...
		fields := rv.NumField()
		// big.Int类型不能当做结构体被识别，否则会出现参数不匹配错误，这里使用直接返回的方式特殊处理
		if rv.Type().String() == "big.Int" {
		...
		if as.IsArray {
			...
			if as.ArrayLength > 0 {
				...
				for n := 0; n < val.Len(); n++ {
					...
					if val.Index(n).Type().String() == "string" {
						s, _ := offset.pack(strOffset)
						strPrefixByte = append(strPrefixByte, s...)
						fixedSize += 256 / 8
					}
					tail = append(tail, d...)
					strOffset += len(d)
				...
				packedDynamic = append(packedDynamic, strPrefixByte...)
				packedDynamic = append(packedDynamic, tail...)
			}
		} else {
			err := addArg(a, as)
			if err != nil {
	...
	for i, as := range argSpec {
		...
		if as.IsArray {
			...
			if as.ArrayLength > 0 {
				...
				// 如果是evm的动态类型
				if as.ArrayLength == 0 && as.IsArray && reflect.TypeOf(as.EVM).String() == "abi.EVMString" {
					skip := int(o) + int(length*32)
					data = append(data[:64], data[skip:]...)
					for i := 0; i < int(length); i++ {
						l, err = UnpackDynamic(data, int(o), intermediate[i])
						if err != nil {
							return err
						}
						o += int64(64)
