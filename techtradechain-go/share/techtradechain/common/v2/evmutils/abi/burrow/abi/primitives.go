...
func (e EVMUint) pack(v interface{}) ([]byte, error) {
	...
	case reflect.String:
		...
		if !ok {
			...
			if reg.MatchString(arg.String()) { // "123" 这种带了引号的数字
				numStr := arg.String()
				numStr = numStr[1 : len(numStr)-1]
				_, ok2 := n.SetString(numStr, 0)
				if !ok2 {
	...
	case reflect.Ptr: //*big.Int
		if num, ok := v.(*big.Int); ok {
			n = num
		}
	case reflect.Struct: //big.Int
		if num, ok := v.(big.Int); ok {
			n = &num
		}
	default:
...
func (e EVMAddress) pack(v interface{}) ([]byte, error) {
	...
	case string:
		a = strings.ToLower(a)
		reg := regexp.MustCompile(`^['"][0-9a-fx]+['"]$`)
		if reg.MatchString(a) { // "1232342342353422" 这种带了引号的地址
			a = a[1 : len(a)-1]
		}
...
func UnpackDynamic(data []byte, offset int, v interface{}) (int, error) {
	lenType := EVMInt{M: 64}
	var length int64
	l, err := lenType.unpack(data, offset, &length)
	if err != nil {
	...
	// 数组长度也会占用32个字节存储所以计算一下
	skipSize := ElementSize
	// 如果字符串不到32个字节就直接返回32字节
	if int(length) < 32 {
		return skipSize, nil
	}
	// 否则跳过32个字节的倍数的大小+存储数组长度的字节的位置
	if int(length)%32 > 0 {
		skipSize += int(length/32)*32 + 32
	}
	return skipSize, nil
}
