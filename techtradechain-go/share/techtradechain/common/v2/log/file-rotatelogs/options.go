...
import (
	...
	"techtradechain.com/techtradechain/common/v2/log/file-rotatelogs/internal/option"
)
...
const (
	...
	optkeyNoCompressCount = "no-compress-count"
	optkeyHmacKey         = "hmac-key"
	optkeyArchivePath     = "archive-path"
...
func WithNoCompressCount(noCompressCount int) Option {
	return option.New(optkeyNoCompressCount, noCompressCount)
}

func WithHmacKey(hmacKey string) Option {
	return option.New(optkeyHmacKey, hmacKey)
}

func WithArchivePath(archivePath string) Option {
	return option.New(optkeyArchivePath, archivePath)
}
