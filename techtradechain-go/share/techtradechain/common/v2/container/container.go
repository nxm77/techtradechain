...
const (
	NoConcreteFound = " no concrete found for: "
	ContainerString = "container:"
	ResolveType     = "resolve type: "
	InvalidAbs      = " invalid abstraction"
)
...
func (c *Container) Register(constructor interface{}, options ...Option) error {
	...
	if reflectedResolver.Kind() != reflect.Func {
		return errors.New(ContainerString + " the constructor must be a function")
	}
...
func (c *Container) arguments(function interface{}, specifiedParameters map[int]interface{},
	...
	for i := 0; i < argumentsCount; i++ {
		...
		if err != nil {
			...
			return nil, errors.New(ResolveType + resolveType + NoConcreteFound + abstraction.String())
		}
...
func (c *Container) getBinding(theType reflect.Type, name string) (*binding, error) {
	if namedBinding, exist := c.bind[theType]; exist {
		...
		if name != "" {
			...
			return nil, errors.New(ContainerString + NoConcreteFound + theType.String() + " name: " + name)
...
func (c *Container) invoke(function interface{}, specifiedParameters map[int]interface{},
	...
	for _, rt := range returns {
		if !rt.IsNil() && rt.Type().Implements(errType) { //返回类型中有不为空的error
			return nil, rt.Interface().(error) //nolint:errcheck
		}
...
func (c *Container) Resolve(abstraction interface{}, options ...ResolveOption) error {
	...
	if receiverType == nil {
		return errors.New(ContainerString + InvalidAbs)
	}
	...
	if receiverType.Kind() == reflect.Ptr {
		...
		if err != nil {
			return errors.New(ResolveType + receiverType.String() + NoConcreteFound + elem.String())
		}
		...
		instance, err := b.resolve(c)
		if err != nil {
			return err //errors.New(ResolveType + receiverType.String() + " " + err.Error())
		}
...
func (c *Container) Call(function interface{}, options ...CallOption) ([]interface{}, error) {
	...
	if receiverType == nil || receiverType.Kind() != reflect.Func {
		return nil, errors.New(ContainerString + " invalid function")
	}
...
func (c *Container) Fill(structure interface{}) error {
	...
	if receiverType == nil {
		return errors.New(ContainerString + " invalid structure")
	}
	...
	if receiverType.Kind() == reflect.Ptr {
		...
		if elem.Kind() == reflect.Struct {
			...
			for i := 0; i < s.NumField(); i++ {
				...
				if !ok {
					...
					return errors.New(ContainerString + NoConcreteFound + f.Type().String())
				}
				if sliceFill {
				...
				if name, exist := s.Type().Field(i).Tag.Lookup("name"); exist {
					if concrete, exist := namedBinding.namedBinding[name]; exist {
						...
						return fmt.Errorf(ContainerString+" cannot resolve %v field", s.Type().Field(i).Name)
					}
				}
		...
		return errors.New(ContainerString + " invalid structure, input elem type:" + elem.Kind().String())
	}

	return errors.New(ContainerString + " invalid structure, input type:" + receiverType.Kind().String())
}
