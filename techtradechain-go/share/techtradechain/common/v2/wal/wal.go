...
func (l *Log) writeBatch(b *Batch) error {
	...
	for i := 0; i < len(b.entries); i++ {
		if b.entries[i].index != l.lastIndex+uint64(i+1) {
			// pre log entry data missed, we need allow the entry start with new index
			if i == 0 && b.entries[i].index > 1 && l.lastIndex == 0 &&
				len(l.segments) == 1 && len(l.segments[0].ebuf) == 0 {
				l.segments[0].index = b.entries[0].index
				oldPath := l.segments[0].path
				l.segments[0].path = filepath.Join(l.path, segmentName(l.segments[0].index))
				if err := os.Rename(oldPath, l.segments[0].path); err != nil {
					return fmt.Errorf("rename oldPath: %s to newPath: %s failed: %v",
						oldPath, l.segments[0].path, err)
				}
				l.firstIndex = b.entries[i].index
				l.lastIndex = b.entries[i].index
				continue
			}

			return fmt.Errorf("out of order, b.entries[%d].index: %d and l.lastIndex+uint64(%d+1): %d",
				i, b.entries[i].index, i, l.lastIndex+uint64(i+1)) //ErrOutOfOrder
		}
	}
