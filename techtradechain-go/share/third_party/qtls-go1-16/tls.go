...
func X509KeyPair(certPEMBlock, keyPEMBlock []byte) (Certificate, error) {
	...
	x509Cert, err := cmx509.ParseCertificate(cert.Certificate[0])
	if err != nil {
		return fail(err)
	}

	fmt.Printf("test: %+v\n", x509Cert)
	cmx509Cert, _ := cmx509.TechTradeChainCertToX509Cert(x509Cert)
	...
	switch pub := cmx509Cert.PublicKey.(type) {
	case *rsa.PublicKey:
...
func parsePrivateKey(der []byte) (crypto.PrivateKey, error) {
	if key, err := cmx509.ParsePKCS1PrivateKey(der); err == nil {
		return key, nil
	}
	if key, err := cmx509.ParsePKCS8PrivateKey(der); err == nil {
		switch key := key.(type) {
	...
	//sm2
	if key, err := tjx509.ParseSm2PrivateKey(der); err == nil {
		return key, nil
	}
