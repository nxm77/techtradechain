...
func removeOpcodeByData(pkscript []parsedOpcode, data []byte) ([]parsedOpcode, bool) {
	// Avoid work when possible.
	if len(pkscript) == 0 || len(data) == 0 {
		return pkscript, false
	}
	...
	for _, pop := range pkscript {
		var found bool
		retScript, found = removeOpcodeCanonical(pop, data, retScript)
		if found {
			match = true
		}
	}

	return retScript, match

}

func removeOpcodeCanonical(t parsedOpcode, dataToRemove []byte,
	result []parsedOpcode) ([]parsedOpcode, bool) {
	var found bool
	...
	if canonicalPush(t) && bytes.Equal(t.data, dataToRemove) {
		found = true
	} else {
