...
import (
	...
	"techtradechain.com/techtradechain-go/tools/cmc/util"
...
func ClientCMD() *cobra.Command {
	...
	clientCmd.AddCommand(getTechTradeChainServerVersionCMD())
...
func init() {
	...
	flags.StringVar(&orgId, flagOrgId, "", "specify the orgId, such as wx-org1.techtradechain.com")
	...
	flags.Int32Var(&addressType, flagAddressType, 0, "address type, eg. TechTradeChain:0, ZXL:1")
...
// getTechTradeChainServerVersionCMD a command about get techtradechain server version
// @return *cobra.Command
func getTechTradeChainServerVersionCMD() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "cmversion",
		Short: "get techtradechain server version",
		Long:  "get techtradechain server version",
		RunE: func(_ *cobra.Command, _ []string) error {
			return getTechTradeChainServerVersion()
		},
	}
...
// getTechTradeChainServerVersion query TechTradeChain server version
// @return error
func getTechTradeChainServerVersion() error {
	...
	defer client.Stop()
	version, err := client.GetTechTradeChainServerVersion()
	if err != nil {
		return fmt.Errorf("get techtradechain server version failed, %s", err.Error())
	}
	fmt.Printf("current techtradechain server version: %s \n", version)
	return nil
}
