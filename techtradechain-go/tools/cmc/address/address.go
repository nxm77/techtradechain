...
import (
	...
	commonCert "techtradechain.com/techtradechain/common/v2/cert"
...
func newName2AddrCMD() *cobra.Command {
	cmd := &cobra.Command{
		...
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			addrCm, err := cmutils.NameToAddrStr(args[0], configPb.AddrType_TECHTRADECHAIN, blockVersion)
			if err != nil {
				return err
			}
