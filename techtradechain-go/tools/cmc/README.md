...
## 简介
cmc`(TechTradeChain Client)`是TechTradeChain提供的命令行工具，用于和TechTradeChain链进行交互以及生成证书等功能。cmc基于go语言编写，通过使用TechTradeChain的go语言sdk（使用grpc协议）达到和TechTradeChain链进行交互的目的。<br>
cmc的详细日志请查看`./sdk.log`
...
```sh
# 创建工作目录
$ mkdir ~/techtradechain
# 编译cmc
$ cd ~/techtradechain
$ git clone -b v2.0.0 https://git.techtradechain.com/techtradechain/techtradechain-go.git
$ cd techtradechain-go/tools/cmc
$ go mod download
$ go build
# 启动测试链
$ cd ~/techtradechain/techtradechain-go/tools
$ git clone -b master https://git.techtradechain.com/techtradechain/techtradechain-cryptogen.git
$ cd techtradechain-cryptogen && make
$ cd ~/techtradechain/techtradechain-go/scripts
$ ./prepare.sh 4 1 # 这里以4组织，每组织1共识节点为例。使用默认参数，一直回车。
$ ./build_release.sh
$ ./cluster_quick_start.sh normal
# 配置测试数据
$ cd ~/techtradechain
$ cp -rf ./techtradechain-go/build/crypto-config ./techtradechain-go/tools/cmc/testdata/ # 使用techtradechain-cryptogen生成的测试链的证书
# 配置完成
$ cd ~/techtradechain/techtradechain-go/tools/cmc
$ ./cmc --help
```
...
编译&配置 步骤使用的是 [SDK配置模版](https://git.techtradechain.com/techtradechain/sdk-go/-/blob/master/testdata/sdk_config.yml) <br>
可通过修改 ~/techtradechain/techtradechain-go/tools/cmc/testdata/sdk_config.yml 实现自定义配置。<br>
...
其他详细配置项请参看 ~/techtradechain/techtradechain-go/tools/cmc/testdata/sdk_config.yml 中的注解。<br>
...
#### 证书管理
  - 生成ca证书
    ...
    **示例**
    ...
    ```sh
    $ ./cmc cert ca -c wx-org1.techtradechain.com -k ca.key -H sha256 --ou root-ca -n ca.crt -p ./ --org wx-org1
    ```
  ...
  - 生成crl列表
    ...
    **示例**
    ...
    ```sh
    $ ./cmc cert crl -C ./ca.crt -K ca.key --crl-path=./client1.crl --crt-path=../sdk/testdata/crypto-config/wx-org1.techtradechain.com/user/client1/client1.tls.crt
    ```
  ...
  - 生成csr文件
    ...
    **示例**
    ...
    ```sh
    $ ./cmc cert csr -c wx-org1.techtradechain.com -k ../sdk/testdata/crypto-config/wx-org1.techtradechain.com/user/client1/client1.tls.key -n client1.csr -p ./ --ou root-ca -o wx-org1
    ```
  ...
  - 根据证书获取节点Id
    ...
    **示例**
    ...
    ```sh
    $ ./cmc cert nid --node-cert-path=../sdk/testdata/crypto-config/wx-org1.techtradechain.com/node/consensus1/consensus1.tls.crt
...
##### 用户合约
  ...
  ```sh
    ...
    --admin-crt-file-paths: admin签名者的tls crt文件的路径列表. 单签模式下只需要填写一个即可, 离线多签模式下多个需要用逗号分割
        比如 ./wx-org1.techtradechain.com/admin1.tls.crt,./wx-org2.techtradechain.com/admin1.tls.crt
    --admin-key-file-paths: admin签名者的tls key文件的路径列表. 单签模式下只需要填写一个即可, 离线多签模式下多个需要用逗号分割
        比如 ./wx-org1.techtradechain.com/admin1.tls.key,./wx-org2.techtradechain.com/admin1.tls.key
  ...
  - 创建wasm合约
    ...
    --admin-key-file-paths=./testdata/crypto-config/wx-org1.techtradechain.com/user/admin1/admin1.tls.key,./testdata/crypto-config/wx-org2.techtradechain.com/user/admin1/admin1.tls.key,./testdata/crypto-config/wx-org3.techtradechain.com/user/admin1/admin1.tls.key \
    --admin-crt-file-paths=./testdata/crypto-config/wx-org1.techtradechain.com/user/admin1/admin1.tls.crt,./testdata/crypto-config/wx-org2.techtradechain.com/user/admin1/admin1.tls.crt,./testdata/crypto-config/wx-org3.techtradechain.com/user/admin1/admin1.tls.crt \
    --sync-result=true \
    --params="{}"
    ```
      ...
      > 如下返回表示成功：
      >
      > response: message:"OK" contract_result:<result:"\n\004fact\022\0031.0\030\002*<\n\026wx-org1.techtradechain.com\020\001\032 F]\334,\005O\200\272\353\213\274\375nT\026%K\r\314\362\361\253X\356*2\377\216\250kh\031" message:"OK" > tx_id:"991a1c00369e4b76853dadf410182bcdfc86062f8cf1478f93482ba9000191d7"
  ...
  - 创建evm合约
    ...
    ```
      ...
      > response: message:"OK" contract_result:<result:"\n(532c238cec7071ce8655aba07e50f9fb16f72ca1\022\0031.0\030\005*<\n\026wx-org1.techtradechain.com\020\001\032 F]\334,\005O\200\272\353\213\274\375nT\026%K\r\314\362\361\253X\356*2\377\216\250kh\031" message:"OK" > tx_id:"e2af1241ff464d47b869a69ce8a615df50da57d3faff4754ad6e45b9f914b938"
  ...
  - 升级合约
    ...
    --admin-key-file-paths=./testdata/crypto-config/wx-org1.techtradechain.com/user/admin1/admin1.tls.key,./testdata/crypto-config/wx-org2.techtradechain.com/user/admin1/admin1.tls.key,./testdata/crypto-config/wx-org3.techtradechain.com/user/admin1/admin1.tls.key \
    --admin-crt-file-paths=./testdata/crypto-config/wx-org1.techtradechain.com/user/admin1/admin1.tls.crt,./testdata/crypto-config/wx-org2.techtradechain.com/user/admin1/admin1.tls.crt,./testdata/crypto-config/wx-org3.techtradechain.com/user/admin1/admin1.tls.crt \
    --org-id=wx-org1.techtradechain.com \
    --sync-result=true \
    --params="{}"
    ```
      ...
      > upgrade contract resp: message:"OK" contract_result:<result:"\n\004fact\022\0032.0\030\002*<\n\026wx-org1.techtradechain.com\020\001\032 F]\334,\005O\200\272\353\213\274\375nT\026%K\r\314\362\361\253X\356*2\377\216\250kh\031" message:"OK" > tx_id:"d89df9fcd87f4071972fdabdf3003a349250a94893fb43899eac4d68e7855d52" 
  ...
  - 冻结合约
    ...
    >
    > freeze contract resp: message:"OK" contract_result:<result:"{\"name\":\"fact\",\"version\":\"3.0\",\"runtime_type\":2,\"status\":1,\"creator\":{\"org_id\":\"wx-org1.techtradechain.com\",\"member_type\":1,\"member_info\":\"Rl3cLAVPgLrri7z9blQWJUsNzPLxq1juKjL/jqhraBk=\"}}" message:"OK" > tx_id:"09841775173548ad9a8a39e2987a4f5115d59d50dd3448e8b09a83624dee5367"
  ...
  - 解冻合约
    ...
    >
    > unfreeze contract resp: message:"OK" contract_result:<result:"{\"name\":\"fact\",\"version\":\"3.0\",\"runtime_type\":2,\"creator\":{\"org_id\":\"wx-org1.techtradechain.com\",\"member_type\":1,\"member_info\":\"Rl3cLAVPgLrri7z9blQWJUsNzPLxq1juKjL/jqhraBk=\"}}" message:"OK" > tx_id:"fccf024450c140dea999cc46ad24d381a679ce2142bd48b2a829abcd4f099866"
  ...
  - 吊销合约
    ...
    > 
    > revoke contract resp: message:"OK" contract_result:<result:"{\"name\":\"fact\",\"version\":\"3.0\",\"runtime_type\":2,\"status\":2,\"creator\":{\"org_id\":\"wx-org1.techtradechain.com\",\"member_type\":1,\"member_info\":\"Rl3cLAVPgLrri7z9blQWJUsNzPLxq1juKjL/jqhraBk=\"}}" message:"OK" > tx_id:"d971b57cf12c46ff8fe0d4f5897634c644fb802998f44360bb130f27ff54a10a"
...
<span id="chainConfig.addrFromCert"></span>
  - 用户证书计算出用户的地址
    ...
    ```sh
    $ ./cmc cert userAddr --cert-path=./testdata/crypto-config/wx-org1.techtradechain.com/user/client1/client1.sign.crt
    ```
...
<span id="chainConfig.dposMint"></span>
  - 增发Token
    ...
    --org-id=wx-org1.techtradechain.com \
    --chain-id=chain1 \
    --user-tlscrt-file-path=./testdata/crypto-config/wx-org1.techtradechain.com/user/client1/client1.tls.crt \
    --user-tlskey-file-path=./testdata/crypto-config/wx-org1.techtradechain.com/user/client1/client1.tls.key \
    --user-signcrt-file-path=./testdata/crypto-config/wx-org1.techtradechain.com/user/client1/client1.sign.crt \
    --user-signkey-file-path=./testdata/crypto-config/wx-org1.techtradechain.com/user/client1/client1.sign.key \
    --sync-result=true
    ```
...
<span id="chainConfig.dposDelegate"></span>
  - 抵押权益到验证人
    ...
    --amount=100000000 \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=wx-org5.techtradechain.com \
    --chain-id=chain1 \
    --user-tlscrt-file-path=./testdata/crypto-config/wx-org5.techtradechain.com/user/client1/client1.tls.crt \
    --user-tlskey-file-path=./testdata/crypto-config/wx-org5.techtradechain.com/user/client1/client1.tls.key \
    --user-signcrt-file-path=./testdata/crypto-config/wx-org5.techtradechain.com/user/client1/client1.sign.crt \
    --user-signkey-file-path=./testdata/crypto-config/wx-org5.techtradechain.com/user/client1/client1.sign.key \
    --sync-result=true
    ```
...
<span id="chainConfig.addOrgRootCA"></span>
  - 增加组织根证书
    ...
    --admin-crt-file-paths=./testdata/crypto-config/wx-org1.techtradechain.com/user/admin1/admin1.tls.crt,./testdata/crypto-config/wx-org2.techtradechain.com/user/admin1/admin1.tls.crt,./testdata/crypto-config/wx-org3.techtradechain.com/user/admin1/admin1.tls.crt \
    --admin-key-file-paths=./testdata/crypto-config/wx-org1.techtradechain.com/user/admin1/admin1.tls.key,./testdata/crypto-config/wx-org2.techtradechain.com/user/admin1/admin1.tls.key,./testdata/crypto-config/wx-org3.techtradechain.com/user/admin1/admin1.tls.key \
    --trust-root-org-id=wx-org5.techtradechain.com \
    --trust-root-path=./testdata/crypto-config/wx-org5.techtradechain.com/ca/ca.crt
    ```
...
<span id="chainConfig.addConsensusNodeOrg"></span>
  - 添加共识节点Org
    ...
    --node-org-id=wx-org5.techtradechain.com
    ```
...
<span id="chainConfig.addConsensusNodeId"></span>
  - 添加共识节点Id
    ...
    --admin-key-file-paths=./testdata/crypto-config/wx-org1.techtradechain.com/user/admin1/admin1.tls.key,./testdata/crypto-config/wx-org2.techtradechain.com/user/admin1/admin1.tls.key,./testdata/crypto-config/wx-org3.techtradechain.com/user/admin1/admin1.tls.key \
    --node-id=QmcQHCuAXaFkbcsPUj7e37hXXfZ9DdN7bozseo5oX4qiC4 \
    --node-org-id=wx-org1.techtradechain.com
    ```
