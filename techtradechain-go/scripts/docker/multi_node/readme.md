...
step1. change `crypto_config[0].count=n` in file `techtradechain-go/tools/techtradechain-cryptogen/config/crypto_config_template.yml`
...
CONFIG_DIR=$4
SERVER_COUNT=$5
IMAGE="techtradechainofficial/techtradechain:v2.3.6.1" # change this
```

step3. prepare 
```sh
cd techtradechain-go/script 
...
sed -i "s%127.0.0.1%192.168.1.35%g" config/node*/techtradechain.yml
