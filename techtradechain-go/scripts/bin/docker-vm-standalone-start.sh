...
EXPOSE_PORT=22351
RUNTIME_PORT=32351
CONTAINER_NAME=techtradechain-vm-go
IMAGE_NAME="techtradechainofficial/techtradechain-vm-engine:v2.3.6"
...
read -r -p "input container name(default 'techtradechain-docker-vm'): " tmp
if  [ -n "$tmp" ] ;then
  CONTAINER_NAME=$tmp
else
  echo "container name use default: 'techtradechain-docker-vm'"
fi
