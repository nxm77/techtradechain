...
config_file="../config/{org_id}/techtradechain.yml"
# config_file="../../config/wx-org1-solo/techtradechain.yml"
eval $(parse_yaml "$config_file" "techtradechain_")

VM_GO_IMAGE_NAME="techtradechainofficial/techtradechain-vm-engine:v2.3.6"
DOCKER_VM_IMAGE_NAME="techtradechainofficial/techtradechain-vm-docker-go:v2.3.1"
START_FULL_MODE=""
...
for i in "$@";do
    case "$i" in
        ...
        *)
            ...
            echo $"  full: start techtradechain node with the vm containers"
            echo $"  alone: start techtradechain node alone, WITHOUT vm containers"
...
function start_vm_go() {
  ...
  mount_path=$techtradechain_vm_go_data_mount_path
  log_path=$techtradechain_vm_go_log_mount_path
  ...
  protocol=$techtradechain_vm_go_protocol
  vm_go_log_level=$techtradechain_vm_go_log_level
  dockervm_config_path=$techtradechain_vm_go_dockervm_config_path
  runtime_server_host=$techtradechain_vm_go_runtime_server_host
  runtime_server_port=$techtradechain_vm_go_runtime_server_port
  contract_engine_port=$techtradechain_vm_go_contract_engine_port
  rpc_timeout=$techtradechain_vm_go_dial_timeout
  rpc_max_send_size=$techtradechain_vm_go_max_send_msg_size
  rpc_max_recv_size=$techtradechain_vm_go_max_recv_msg_size
  log_in_console=$techtradechain_vm_go_log_in_console
  max_concurrency=$techtradechain_vm_go_max_concurrency
  slow_disable=$techtradechain_vm_go_slow_disable
  slow_step_time=$techtradechain_vm_go_slow_step_time
  slow_tx_time=$techtradechain_vm_go_slow_tx_time
  process_timeout=$techtradechain_vm_go_process_timeout
  cgroup_disable=$techtradechain_vm_common_contract_engine_cgroup_disable
  max_mem_size_per_process=$techtradechain_vm_common_contract_engine_cgroup_max_mem_size_per_process
  max_cpu_percent_per_process=$techtradechain_vm_common_contract_engine_cgroup_max_cpu_percent_per_process
  devices_allow=$techtradechain_vm_common_contract_engine_cgroup_devices_allow
  devices_deny=$techtradechain_vm_common_contract_engine_cgroup_devices_deny
...
function start_docker_vm_go() {
  ...
  # concat mount_path and log_path for container to mount
  docker_vm_mount_path=$techtradechain_vm_docker_go_dockervm_mount_path
  docker_vm_log_path=$techtradechain_vm_docker_go_dockervm_log_path
  docker_vm_log_level=$techtradechain_vm_docker_go_log_level
  docker_vm_log_in_console=$techtradechain_vm_docker_go_log_in_console
...
pid=$(ps -ef | grep techtradechain | grep "\-c ../config/{org_id}/techtradechain.yml {tagName}" | grep -v grep |  awk  '{print $2}')
if [ -z "${pid}" ];then
    # check if enable go vm
    if [[ $techtradechain_vm_go_enable == "true" ]]; then
     enable_go_vm_container=true
    fi
    # check if enable Deprecated docker vm
    if [[ $techtradechain_vm_docker_go_enable_dockervm == "true" && $techtradechain_vm_docker_go_uds_open == "true" ]]; then
    ...
    #nohup ./techtradechain start -c ../config/{org_id}/techtradechain.yml {tagName} > /dev/null 2>&1 &
    nohup ./techtradechain start -c ../config/{org_id}/techtradechain.yml {tagName} >> panic.log 2>&1 &
