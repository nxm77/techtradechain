...
/*
sql rust test/wasm/rust-sql-2.0.0.wasm 源码所在目录：techtradechain-contract-sdk-rust v2.0.0_dev src/contract_fact_sql.rs
sql tinygo go-test/wasm/sql-2.0.0.wasm 源码所在目录：techtradechain-contract-sdk-tinygo develop demo/main_fact_sql.go
...
import (
	...
	"techtradechain.com/techtradechain-go/test/common"
...
const (
	...
	orgId          = "wx-org1.techtradechain.com"
	prePathFmt     = certPathPrefix + "/crypto-config/wx-org%s.techtradechain.com/user/admin1/"

	IPOrg2          = "localhost"
	PortOrg2        = 12352
	userKeyPathOrg2 = certPathPrefix + "/crypto-config/wx-org2.techtradechain.com/user/client1/client1.sign.key"
	userCrtPathOrg2 = certPathPrefix + "/crypto-config/wx-org2.techtradechain.com/user/client1/client1.sign.crt"
	orgIdOrg2       = "wx-org2.techtradechain.com"
	prePathFmtOrg2  = certPathPrefix + "/crypto-config/wx-org%s.techtradechain.com/user/admin1/"
)
...
func initClientOrg2(sk3 crypto.PrivateKey, client *apiPb.RpcNodeClient) (isSolo bool) {
	...
	// init client org2
	caPaths = append(caPaths, certPathPrefix+"/crypto-config/wx-org2.techtradechain.com/ca")
...
func functionalTest(sk3 crypto.PrivateKey, client *apiPb.RpcNodeClient) {
	...
	fmt.Println("// 4) 执行合约-sql update name=技术贸易链techtradechain_update where id="+id, time.Now().Format("2006-01-02 15:04:05"))
	txId = testInvokeSqlUpdate(sk3, client, CHAIN1, id)
	testWaitTx(sk3, clientOrg2, CHAIN1, txId)

	fmt.Println("// 5) 查询 id="+id+" 看name是不是更新成了技术贸易链techtradechain_update：", time.Now().Format("2006-01-02 15:04:05"))
	_, result = testQuerySqlById(sk3, clientOrg2, CHAIN1, id)
	json.Unmarshal([]byte(result), &rs)
	fmt.Println("testInvokeSqlUpdate query", rs)
	if string(rs["name"]) != "技术贸易链techtradechain_update" {
		fmt.Println("result", rs)
	...
	id = txId
	for i := 0; i < 3; i++ {
		fmt.Println("试图将id="+id+" 的name改为技术贸易链techtradechain_save_point，但是发生了错误，所以修改不会成功", time.Now().Format("2006-01-02 15:04:05"))
		...
		if string(rs["name"]) == "techtradechain_save_point" {
			panic("testInvokeSqlUpdateRollbackDbSavePoint test 【fail】 query by id error, age err")
		} else if string(rs["name"]) == "技术贸易链techtradechain" {
	...
	if runtimeType == commonPb.RuntimeType_WASMER {
		...
		panicNotEqual(result, "")
		time.Sleep(time.Millisecond * 200)
		fmt.Println("\n// 3、SQL中，禁止使用事务相关操作的语句，比如commit 、rollback等，事务由TechTradeChain框架自动控制。")
...
func testInvokeSqlInsert(sk3 crypto.PrivateKey, client *apiPb.RpcNodeClient, chainId string, age string, print bool) string {
	...
	pairs := []*commonPb.KeyValuePair{
		...
		{
			Key:   "name",
			Value: []byte("技术贸易链techtradechain"),
		},
		{
			Key:   "id_card_no",
...
func testInvokeSqlUpdate(sk3 crypto.PrivateKey, client *apiPb.RpcNodeClient, chainId string, id string) string {
	...
	pairs := []*commonPb.KeyValuePair{
		...
		{
			Key:   "name",
			Value: []byte("技术贸易链techtradechain_update"),
		},
	}
	payload := &commonPb.Payload{
		ContractName: contractName,
...
func testInvokeSqlUpdateRollbackDbSavePoint(sk3 crypto.PrivateKey, client *apiPb.RpcNodeClient, chainId string, id string) string {
	...
	pairs := []*commonPb.KeyValuePair{
		...
		{
			Key:   "name",
			Value: []byte("techtradechain_save_point"),
		},
	}
	payload := &commonPb.Payload{
		ContractName: contractName,
...
//		// 构造Sender
//		sender1 := &acPb.Member{
//			OrgId:      "wx-org" + numStr + ".techtradechain.com",
//			MemberInfo: file2,
//			//IsFullCert: true,
//		}
//
